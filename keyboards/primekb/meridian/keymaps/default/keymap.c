...
//Initialize pin B12 for in-swithc caps lock indication
...
void keyboard_post_init_user(void) {
    rgblight_setrgb_at(0, 0, 0, 0); // (x, x, x, 0) = top LED
    rgblight_setrgb_at(0, 0, 0, 1); // (x, x, x, 1) = middle LED
    rgblight_setrgb_at(0, 0, 0, 2); // (x, x, x, 2) = bottom LED
}
