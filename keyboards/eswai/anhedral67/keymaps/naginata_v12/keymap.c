...
/* _SHIFT
  ...
  |   ~   |   !   |   @   |   #   |   $   |   %   |   ^   |   &   |   *   |   (   |   )   |   _   |   =   |  __   |  __   |
  +-------+-------+-------+-------+-------+-------+-------+-------+-------+-------+-------+-------+-------+-------+-------+
  |S(ESC) | S(Q)  | S(G)  | S(M)  | S(L)  | S(W)  |   ;   |       | S(Y)  | S(F)  | S(U)  | S(B)  | S(H)  |S(BSPC)|       |
  +-------+-------+-------+-------+-------+-------+-------+-------+-------+-------+-------+-------+-------+-------+-------+
  |S(TAB) | S(D)  | S(S)  | S(T)  | S(N)  | S(R)  |   |   |       | S(I)  | S(A)  | S(E)  | S(O)  |   "   |S(ENT) |       |
  +-------+-------+-------+-------+-------+-------+-------+-------+-------+-------+-------+-------+-------+-------+-------+
  |  __   | S(Z)  | S(X)  | S(C)  | S(V)  | S(J)  |   <   |   >   | S(K)  | S(P)  |   <   |   >   |   /   | S(UP) |S(DEL) |
  +-------+-------+-------+-------+-------+-------+-------+-------+-------+-------+-------+-------+-------+-------+-------+
  |  __   |       |       | LGUI  |  __   |  __   |  __   |       |  __   |  __   |  __   |       |S(LEFT)|S(DOWN)|S(RGHT)|
  +-------+-------+-------+-------+-------+-------+-------+-------+-------+-------+-------+-------+-------+-------+-------+
*/
  [_SHIFT] = LAYOUT(
    JP_TILD   ,JP_EXLM   ,JP_AT     ,JP_HASH   ,JP_DLR    ,JP_PERC   ,JP_CIRC   ,JP_AMPR   ,JP_ASTR   ,JP_LPRN   ,JP_RPRN   ,JP_UNDS   ,JP_EQL    ,_______   ,_______   , \
    S(KC_ESC) ,S(KC_Q)   ,S(KC_G)   ,S(KC_M)   ,S(KC_L)   ,S(KC_W)   ,JP_SCLN   ,           S(KC_Y)   ,S(KC_F)   ,S(KC_U)   ,S(KC_B)   ,S(KC_H)   ,S(KC_BSPC)        , \
    S(KC_TAB) ,S(KC_D)   ,S(KC_S)   ,S(KC_T)   ,S(KC_N)   ,S(KC_R)   ,JP_PIPE   ,           S(KC_I)   ,S(KC_A)   ,S(KC_E)   ,S(KC_O)   ,JP_DQT    ,S(KC_ENT)         , \
    _______   ,S(KC_Z)   ,S(KC_X)   ,S(KC_C)   ,S(KC_V)   ,S(KC_J)   ,JP_LT     ,JP_GT     ,S(KC_K)   ,S(KC_P)   ,JP_LT     ,JP_GT     ,JP_SLSH   ,S(KC_UP)  ,S(KC_DEL) , \
...
/* _LOWER
  ...
  +------+------+------+------+------+------+------+------+------+------+------+------+------+------+------+
  |  __  | C(Z) |  XX  |  XX  |  XX  | C(V) |  [   |  ]   |  0   |  1   |  2   |  3   |  =   | PGUP |  __  |
  +------+------+------+------+------+------+------+------+------+------+------+------+------+------+------+
...
*/
  [_LOWER] = LAYOUT(
    ...
    _______  ,C(KC_Z)  ,XXXXXXX  ,XXXXXXX  ,XXXXXXX  ,C(KC_V)  ,JP_LBRC  ,JP_RBRC  ,KC_0     ,KC_1     ,KC_2     ,KC_3     ,JP_EQL   ,KC_PGUP  ,_______  , \
