...
bool terminate_case_modes(uint16_t keycode, const keyrecord_t *record) {
    switch (keycode) {
        ...
        case KC_BSPC:
        case KC_DOT:
        case KC_NUBS:   // pipe in UK
        case KC_SLSH: 
            // If mod chording disable the mods
            if (record->event.pressed) {
                uint8_t mods = get_mods();
                // if only shift pressed - OK as we are capitalizing
            ...
            break; 
            /* with this the lights seem very off?
        case QK_LAYER_TAP ... QK_LAYER_TAP_MAX:
        case QK_TAP_DANCE ... QK_TAP_DANCE_MAX:
            // for layers ignore  - TODO do we need to look at keycode & 0xFF;
            return false;
            break;
            */
        default:
...
bool process_case_modes(uint16_t keycode, const keyrecord_t *record) {
    if (caps_word_on || xcase_state) {
        if ((QK_MOD_TAP <= keycode && keycode <= QK_MOD_TAP_MAX)
            ...
            // pogues XXX check for tap dance too as i only use for layers
            || (QK_TAP_DANCE <= keycode && keycode <= QK_TAP_DANCE_MAX)) {
