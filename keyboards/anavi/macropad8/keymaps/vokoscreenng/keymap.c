 /* Copyright 2023 Marc Nause <marc.nause@gmx.de>
  *
...
/*
 * The keymap contains 1 layer for vokoscreenNG hotkeys and a 2nd layer
 * for controlling the backlighting and the underlighting.
 *
 * vokoscreenNG does not support hotkeys in the experimental Wayland
 * version yet (at least not in version 3.7.0)
 *
 * - Layer for vokoscreenNG default hotkeys:
 * CTRL + Shift + F10    - Start / Stop
 * CTRL + Shift + F12    - Pause / Continue 
 * CTRL + Shift + F9     - Magnification
 * CTRL + Shift + F8     - Camera
 * CTRL + Shift + F7     - ShowClick
 * CTRL + Shift + F6     - Halo
 * CTRL + Shift + F5     - Snapshot
 *
 */
 ...
 * |---------+---------+--------+---------+
 * |         |         |        |   TO    |
 * |ShowClick|   Halo  |Snapshot|   _FN   |
 * |         |         |        |         |
 *  `-------------------------------------'
 */ 
  [_VOKO] = LAYOUT_ortho_2x4(
     RCS(KC_F10), RCS(KC_F12), RCS(KC_F9), RCS(KC_F8),
     RCS(KC_F7), RCS(KC_F6), RCS(KC_F5), TO(_FN)
  ),
...
bool oled_task_user(void) {
  ...
  switch (get_highest_layer(layer_state)) {
    case _VOKO:
      oled_write_ln_P(PSTR("vokoscreenNG"), false);
      break;
    case _FN:
