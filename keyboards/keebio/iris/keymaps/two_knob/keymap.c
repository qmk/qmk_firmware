 ...
 * 
 * If you make changes and download the json, you can run qmk json2c two_knob.json
 * to get converted text. You can copy and paste it here.
 ...
 * This option has already been enabled in ../../rev6a/config.h
...
/**
 * Set all keys on the current layer to a single color.
 *
 * Only affects layer 1. (Raise).
 */
    ...
    switch(biton32(layer_state)) {
      ...
      case 1:
        if (clockwise) {
          ...
          tap_code(KC_PGDN); 
          // Scrolls down with mouse wheel. (Increments are small, this is slow).
          // tap_code(KC_WH_D);
        } else {
          // Scrolls up. (Page Up).
          tap_code(KC_PGUP);
          // Scrolls up with mouse wheel. (Increments are small, this is slow).
          // tap_code(KC_WH_U);
        }
        break;
      default:
