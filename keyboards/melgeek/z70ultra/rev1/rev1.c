...
bool led_update_kb(led_t led_state) {
    if (led_update_user(led_state)) {
        if (led_state.caps_lock) {
            IS31FL3741_set_pwm_buffer(&g_is31_leds[71], 0xff, 0x00, 0x00);
            IS31FL3741_set_pwm_buffer(&g_is31_leds[72], 0xff, 0x00, 0x00);
        } else {
            IS31FL3741_set_pwm_buffer(&g_is31_leds[71], 0x00, 0x00, 0x00);
            IS31FL3741_set_pwm_buffer(&g_is31_leds[72], 0x00, 0x00, 0x00);
        }

        if (led_state.num_lock) {
            IS31FL3741_set_pwm_buffer(&g_is31_leds[70], 0x00, 0xff, 0x00);
            IS31FL3741_set_pwm_buffer(&g_is31_leds[73], 0x00, 0xff, 0x00);
        } else {
            IS31FL3741_set_pwm_buffer(&g_is31_leds[70], 0x00, 0x00, 0x00);
            IS31FL3741_set_pwm_buffer(&g_is31_leds[73], 0x00, 0x00, 0x00);
        }

        if (led_state.scroll_lock) {
            IS31FL3741_set_pwm_buffer(&g_is31_leds[69], 0x00, 0x00, 0xff);
            IS31FL3741_set_pwm_buffer(&g_is31_leds[74], 0x00, 0x00, 0xff);
        } else {
            IS31FL3741_set_pwm_buffer(&g_is31_leds[69], 0x00, 0x00, 0x00);
            IS31FL3741_set_pwm_buffer(&g_is31_leds[74], 0x00, 0x00, 0x00);
        }
    }
    return true;
}
...
void matrix_init_kb(void) {
    for (int i = AXIS_LED_TOTAL; i < DRIVER_LED_TOTAL; ++i) {
