...
static void rgb_matrix_driver_init(void) {
    ...
    is31fl3741_init(0);
    ws2812_init();
    for (uint8_t index = 0; index < IS31FL3741_LED_COUNT; index++) {
        bool enabled = true;
...
static void rgb_matrix_driver_set_color(int index, uint8_t red, uint8_t green, uint8_t blue) {
    if (index < IS31FL3741_LED_COUNT) {
        is31fl3741_set_color(index, red, green, blue);
    } else {
#    if WS2812_LED_TOTAL > 0
        rgb_matrix_ws2812_array[index - IS31FL3741_LED_COUNT].r = red;
        rgb_matrix_ws2812_array[index - IS31FL3741_LED_COUNT].g = green;
        rgb_matrix_ws2812_array[index - IS31FL3741_LED_COUNT].b = blue;
#    endif
    }
}
