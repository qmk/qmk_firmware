...
/**
 * TS Southpaw RGB Effects System - Version 2.0
 */
 ...
 */
static struct {
    bool enabled;        // Whether the effect is enabled (toggled by FN+E)
    bool active;         // Whether ripple is currently animating
    uint8_t step;
    uint16_t timer;
    bool expanding;
} ripple_state = {true, false, 0, 0, true};  // Enabled by default
...
/**
 * Update indicator state based on activity
 *
 ...
 *
 * Note: Active when Num Lock is OFF (inverse logic for numpad)
 */
...
void toggle_esc_ripple_effect(void) {
    ripple_state.enabled = !ripple_state.enabled;
}
...
void start_esc_ripple_effect(void) {
    if (!ripple_state.enabled) return;  // Don't start if disabled
