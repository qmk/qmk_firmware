...
#define INVERSE_NUMLOCK_LED     true  // Num Lock indicator lights up when NUM Lock is NOT on, set to false if you want the opposite effect
#define TD_LSFT_CAPSLOCK_ENABLE        // Delete this line to disable double tap Left Shift to turn on CAPS LOCK
...
//      Ct_L     Win_L    Alt_L                               SPACE                               Alt_R    FN       Ct_R     Left     Down     Right
   ...
   [_MO3] = LAYOUT(
    ...
    [_COLEMAK] = LAYOUT(
        ...
        TT(_MO2), KC_A,   KC_R,    KC_S,    KC_T,    KC_D,    KC_H,    KC_N,    KC_E,    KC_I,    KC_O,    KC_QUOT,          KC_ENT,           KC_PGDN,
        KC_LSFTCAPS,      KC_Z,    KC_X,    KC_C,    KC_V,    KC_B,    KC_K,    KC_M,    KC_COMM, KC_DOT,  KC_SLSH,          KC_RSFT, KC_UP,   KC_END,
        KC_LCTL, KC_LGUI, KC_LALT,                            KC_SPC,                             KC_RALT, MO(_FN1),KC_RCTL, KC_LEFT, KC_DOWN, KC_RGHT
    ),
};
...
void matrix_scan_user(void) {
    ...
    void rgb_matrix_indicators_advanced_user(uint8_t led_min, uint8_t led_max) {
        ...
        if ( (INVERSE_NUMLOCK_LED != IS_HOST_LED_ON(USB_LED_NUM_LOCK))) {   // Turn on if NUM lock is the opposite of INVERSE_NUMLOCK_LED
        ...
        case _MO3:
            ...
            break;
        case _COLEMAK:
            for (uint8_t i=0; i<ARRAYSIZE(LED_LIST_ALPHABETS); i++) {
                rgb_matrix_set_color(LED_LIST_ALPHABETS[i], RGB_BLUE);
            }
            break;
        default:
            break;
