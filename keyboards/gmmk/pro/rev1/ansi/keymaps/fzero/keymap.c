/* Copyright 2021 Glorious, LLC <salman@pcgamingrace.com>
   ...
   Copyright 2022 Fabio Neves <fabio.fzero@gmail.com>
...
// ---- The mapping cheat-sheet below matches the README picture --------------------------------------------------------------------------------------
//      ESC      F1       F2       F3       F4       F5       F6       F7       F8       F9       F10      F11      F12	     Prt           Rotary(Mute)
//      ~        1        2        3        4        5        6        7        8        9        0         -       (=)	     BackSpc           PgUp
//      Tab      Q        W        E        R        T        Y        U        I        O        P        [        ]        \                 PgDn
//      Caps     A        S        D        F        G        H        J        K        L        ;        "                 Enter             Home
//      Sh_L              Z        X        C        V        B        N        M        ,        .        ?                 Sh_R     Up       End
//      Ct_L     Alt-L    Win_L                               SPACE                               Win_R    Alt-R    Fn       Left     Down     Right
    ...
    [MAC_BASE] = LAYOUT( // macOS layout - press knob to switch to Windows
        ...
        KC_LCTL, KC_LALT, KC_LGUI,                            KC_SPC,                             KC_RGUI, KC_RALT, MO_MACF, KC_LEFT, KC_DOWN, KC_RGHT
    ),

    [MAC_FN] = LAYOUT( // fn modifier for macOS
        _______, KC_BRID, KC_BRIU, KC_MCTL, KC_LPAD, _______, _______, KC_MPRV, KC_MPLY, KC_MNXT, KC_MUTE, KC_VOLD, KC_VOLU, MAC_PS2,          _______,
        ...
        _______,          _______, RGB_HUI, _______, _______, _______, NK_TOGG, _______, ERB,     ERB2,    HTML,             _______, RGB_MOD, _______,
        _______, _______, _______,                            _______,                            _______, _______, _______, RGB_SPD, RGB_RMOD, RGB_SPI
    ),

    [WIN_BASE] = LAYOUT( // Windows layout - press knob to change back to macOS. Swaps left Win key with Control and a few other things
        KC_ESC,  KC_F1,   KC_F2,   KC_F3,   KC_F4,   KC_F5,   KC_F6,   KC_F7,   KC_F8,   KC_F9,   KC_F10,  KC_F11,  KC_F12,  KC_PSCR,          TO_MACB,
        KC_GRV,  KC_1,    KC_2,    KC_3,    KC_4,    KC_5,    KC_6,    KC_7,    KC_8,    KC_9,    KC_0,    KC_MINS, KC_EQL,  KC_BSPC,          KC_PGUP,
        KC_TAB,  KC_Q,    KC_W,    KC_E,    KC_R,    KC_T,    KC_Y,    KC_U,    KC_I,    KC_O,    KC_P,    KC_LBRC, KC_RBRC, KC_BSLS,          KC_PGDN,
        KC_CAPS, KC_A,    KC_S,    KC_D,    KC_F,    KC_G,    KC_H,    KC_J,    KC_K,    KC_L,    KC_SCLN, KC_QUOT,          KC_ENT,           KC_HOME,
        KC_LSFT,          KC_Z,    KC_X,    KC_C,    KC_V,    KC_B,    KC_N,    KC_M,    KC_COMM, KC_DOT,  KC_SLSH,          KC_RSFT, KC_UP,   KC_END,
        KC_LCTL, KC_RALT, KC_RCTL,                            KC_SPC,                             KC_LGUI, KC_LALT, MO_WINF, KC_LEFT, KC_DOWN, KC_RGHT
    ),

    [WIN_FN] = LAYOUT( // fn modifier for Windows
        ...
        KC_LCTL, KC_LALT, KC_LGUI,                            _______,                            KC_RGUI, KC_RALT, _______, RGB_SPD, RGB_RMOD, RGB_SPI
    ),
};
// clang-format on
...
const uint16_t PROGMEM encoder_map[][NUM_ENCODERS][2] = {
    ...
    [MAC_FN]   = { ENCODER_CCW_CW(KC_UP, KC_DOWN) },
    [WIN_BASE] = { ENCODER_CCW_CW(KC_MS_WH_UP, KC_MS_WH_DOWN) },
    [WIN_FN]   = { ENCODER_CCW_CW(KC_UP, KC_DOWN) }
};
#endif
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    switch (keycode) {
    case MAC_PS: // macOS region screenshot
        if (record->event.pressed) {
            SEND_STRING(SS_DOWN(X_LGUI) SS_DOWN(X_LSFT) SS_TAP(X_4) SS_UP(X_LSFT) SS_UP(X_LGUI));
        }
        break;

    case MAC_PS2: // macOS fancy region screenshot/screen recording
        if (record->event.pressed) {
            SEND_STRING(SS_DOWN(X_LGUI) SS_DOWN(X_LSFT) SS_TAP(X_5) SS_UP(X_LSFT) SS_UP(X_LGUI));
        }
        break;

    case ERB: // I'm a Rails dev
        if (record->event.pressed) {
