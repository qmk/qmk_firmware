...
void rgb_matrix_indicators_advanced_user(uint8_t led_min, uint8_t led_max) {
	...
	for (uint8_t i = 0; i < sizeof(right_side_leds) / sizeof(right_side_leds[0]); i++) {
	    rgb_matrix_set_color(right_side_leds[i], tempRGB.r, tempRGB.g, tempRGB.b);
	}
    }

    /*if (IS_HOST_LED_ON(USB_LED_CAPS_LOCK)) {
       int left_side_leds[8] = {68, 71, 74, 77, 81, 84, 88, 92};
       int right_side_leds[8] = {69, 72, 75, 78, 82, 85, 89, 93};
       int i;
       for (i = 0; i < 8; i++) {
