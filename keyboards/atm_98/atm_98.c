...
#ifndef FACTORY_DEFAULT_RGB_MODE
#    define FACTORY_DEFAULT_RGB_MODE RGB_MATRIX_CYCLE_LEFT_RIGHT
#endif
#ifndef FACTORY_DEFAULT_RGBLIGHT_MODE
#    define FACTORY_DEFAULT_RGBLIGHT_MODE 14  // Rainbow swirl 6
#endif
...
#ifndef user_color_t_defined
#    define user_color_t_defined
typedef struct {
    uint8_t r;
    uint8_t g;
    uint8_t b;
...
/* ========== EEPROM 配置（合并 user_config + confinfo） ========== */
typedef union {
    uint32_t raw;
    struct {
        uint8_t mac_mode    : 1;
...
/* 物理三档开关消抖 —— 文件作用域，供 post_init 和 dip_switch 共用 */
static uint32_t dip_debounce_timer = 0;
static uint8_t  dip_pending_state  = 0xFF;
...
bool process_record_kb(uint16_t keycode, keyrecord_t *record) {
    ...
    if (key_state.fn_pressed && record->event.pressed) {
        switch (keycode) {
            case KC_F1: current_encoder_mode = ENCODER_MODE_VOLUME;     return false;
            case KC_F2: current_encoder_mode = ENCODER_MODE_PAGE;       return false;
            case KC_F3: current_encoder_mode = ENCODER_MODE_BRIGHTNESS; return false;
        }
    }

    // Win 锁（非 Mac 模式 + MO(1) 按下时切锁）
    ...
    // 复制/粘贴/剪切
    if (keycode == KC_COPY) {
        if (record->event.pressed) { register_code(KC_LCTL); tap_code(KC_C); unregister_code(KC_LCTL); }
        return false;
    } else if (keycode == KC_PASTE || keycode == KC_PSTE) {
        if (record->event.pressed) { register_code(KC_LCTL); tap_code(KC_V); unregister_code(KC_LCTL); }
        return false;
    } else if (keycode == KC_CUT) {
        if (record->event.pressed) { register_code(KC_LCTL); tap_code(KC_X); unregister_code(KC_LCTL); }
        return false;
    }
    ...
    if (key_state.mac_mode) {
        switch (keycode) {
            case KC_LGUI:
                if (record->event.pressed) register_code(KC_LALT); else unregister_code(KC_LALT);
                return false;
            case KC_LALT:
                if (record->event.pressed) register_code(KC_LGUI); else unregister_code(KC_LGUI);
                return false;
            case KC_RGUI:
                if (record->event.pressed) register_code(KC_RALT); else unregister_code(KC_RALT);
                return false;
            case KC_RALT:
                if (record->event.pressed) register_code(KC_RGUI); else unregister_code(KC_RGUI);
                return false;
        }
    }
    ...
    switch (keycode) {
        case UG_NEXT:
            if (record->event.pressed) rgblight_step();
            return false;
        case UG_PREV:
            if (record->event.pressed) rgblight_step_reverse();
            return false;
        case RM_HUEU:
            if (record->event.pressed) { rgb_matrix_increase_hue(); eeconfig_update_rgblight_current(); }
            return false;
        default:
            return true;
    }
...
bool dip_switch_update_user(uint8_t index, bool active) {
    ...
    if (local_dip_state != last_dip_state) {
        dip_debounce_timer = timer_read32();
        dip_pending_state  = local_dip_state;
    }

    if (dip_pending_state != 0xFF && dip_pending_state != last_dip_state && timer_elapsed32(dip_debounce_timer) >= 50) {
        /* 编码器层切换（随三档开关位置映射） */
        switch (dip_pending_state) {
            case 0: encoder_layer = 2; break;
            case 1: encoder_layer = 0; break;
            case 2: encoder_layer = 1; break;
        }

        last_dip_state     = dip_pending_state;
        dip_pending_state  = 0xFF;
    }
...
bool encoder_update_kb(uint8_t index, bool clockwise) {
    ...
    if (index == 0) {
        ...
        if (IS_QK_MACRO(keycode) || IS_RGB_KEYCODE(keycode) || IS_RGB_MATRIX_KEYCODE(keycode) ||
            ...
            keyrecord_t rec = {
                .event   = { .key = {.row = 0xFF, .col = 0xFF}, .pressed = true, .time = timer_read() },
                .keycode = keycode
            };
...
bool rgb_matrix_indicators_advanced_kb(uint8_t led_min, uint8_t led_max) {
    ...
    if (rgb_matrix_get_mode() == CUSTOM_RGB_MODE) {
        ...
        // Caps Lock 指示灯 + 星环（编码器旋转时星环不覆盖）
        if (host_keyboard_led_state().caps_lock) {
            rgb_matrix_set_color(52, 0xFF, 0xFF, 0xFF);
            if (!encoder_light.active) {
...
#    define GEN_OFF_COLOR(var, shift1, shift2) do { \
    ...
    // 编码器旋转灯效（优先渲染，Caps 指示器仅在编码器不活跃时接管星环）
    if (encoder_light.active && encoder_light_enabled) {
        ...
        uint32_t enc_elapsed = timer_elapsed32(encoder_light.timer);
        if (enc_elapsed >= 500) {
            ...
            uint32_t angle = (enc_elapsed * 720) / 500;
            if (encoder_light.direction) encoder_light.pos = (9 - ((angle / 40) % 9)) % 9;
            else                         encoder_light.pos = (angle / 40) % 9;
            ...
            for (uint8_t j = 0; j < 3; j++) {
                ...
                hsv_t   color_hsv  = {hue, sat, brightness};
                rgb_t   rgb        = hsv_to_rgb(color_hsv);
    ...
    // Caps Lock 指示灯 (52 + 98-106 星环)
    // 编码器旋转时星环由 encoder_light 灯效接管，Caps 指示器不覆盖
...
void housekeeping_task_user(void) {
    ...
    /* USB 状态防抖：仅在 usb_active 稳定 300ms 后才触发断连逻辑，
     * 避免枚举期间的 SUSPEND/ACTIVE 跳变导致误断电/反复切模式 */
    static uint32_t usb_inactive_since = 0;
    if (!usb_active) {
        if (!usb_inactive_since) usb_inactive_since = timer_read32();
    } else {
        usb_inactive_since = 0;
    }
    bool usb_lost = (usb_inactive_since && timer_elapsed32(usb_inactive_since) >= 300);

    if (usb_enumerated && usb_lost) {
        usb_inactive_since = 0;
