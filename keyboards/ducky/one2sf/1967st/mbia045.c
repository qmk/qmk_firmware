...
const uint16_t CIE1931_16_CURVE[] = {
...
void MBIA045_flush(void) {
    for (uint8_t row = 0; row < MATRIX_ROWS; row++) {
        for (uint8_t col = 0; col < MATRIX_COLS; col++) {
            ...
            mbi_color color = g_led_buffer[led_index];

            // Convert color values from 8 to 16 bit
            uint16_t cur_r = CIE1931_16_CURVE[color.r];
            uint16_t cur_g = CIE1931_16_CURVE[color.g];
            uint16_t cur_b = CIE1931_16_CURVE[color.b];

            // Construct the pwm buffer
            for (int i = 0; i < 16; i++) {
                g_pwm_buffer[row][i + (col * 16)] = ((0x1u & (cur_r >> 15)) << 14) | ((0x1u & (cur_g >> 15)) << 13) | ((0x1u & (cur_b >> 15)) << 12);
                cur_r <<= 1;
                cur_g <<= 1;
                cur_b <<= 1;
            }
