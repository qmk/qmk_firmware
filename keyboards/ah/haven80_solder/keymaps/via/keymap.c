...
void freather_set_color(bool capslock_is_on)
{
  ...
  {
    rgblight_sethsv_range(freather.caps.h, freather.caps.s, freather.caps.v,0,21); //0 is the location of caps light indicator
  }
  else{
  ...
  switch (keycode) {
    case KC_CAPS:
      if (record->event.pressed && caps_is_active == false &&freather.caps.enabled == true) {  //turn on caps
        ...
        rgblight_mode(1);
        rgblight_sethsv_range(freather.caps.h, freather.caps.s, freather.caps.v, 0, 21); //needs a way to override the effect
        return true;
      }else if(record->event.pressed && caps_is_active == true ){
        rgblight_sethsv_at(freather.caps.h, freather.caps.s, 0,0);
        rgblight_set_effect_range(1, 20);
        rgblight_mode(mode);
