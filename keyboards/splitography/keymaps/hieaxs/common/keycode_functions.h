...
void modifier(void (*f)(uint8_t))
{
  ...
  if (mods & MOD_BIT(KC_RSFT)) {
    (*f)(KC_LSFT);                          // macros all use left modifiers
  }
}
...
{
  tap_shift(state, KC_SPC, _LSYMBOL);
}

void space_reset(qk_tap_dance_state_t *state, void *user_data)
{
  tap_reset(KC_SPC, _LSYMBOL);
}
...
#define LSHIFT MOD_BIT(KC_LSFT)
#define RSHIFT MOD_BIT(KC_RSFT)
...
void symbol_shift(keyrecord_t *record, uint16_t keycode)
{
  if (keycode) {
    ...
    // a non-shift modifier disables shift layer
    layer_off(_LSHIFT);
    layer_off(_RSHIFT);
