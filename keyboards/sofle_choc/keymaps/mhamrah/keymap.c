...
// Radar sweep animation frames for 128x32 OLED (512 bytes each)
static const uint8_t PROGMEM radar_frame1[] = {
    // Frame 1: Sweep at top right
...
static const uint8_t PROGMEM radar_frame2[] = {
    // Frame 2: Sweep at bottom right
...
static const uint8_t PROGMEM radar_frame3[] = {
    // Frame 3: Sweep at bottom left
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0xFF, 0xFF,
...
#define GIF_FRAME_COUNT 4
#define GIF_FRAME_DURATION_MS 200  // 200ms per frame
    ...
    if (is_keyboard_master()) {
        ...
        switch (current_layer) {
            case _MAC_DEFAULT:
                ...
                oled_set_cursor(1, 3);  // Center "Main" (4 chars, so start at column 1)
                oled_write_P(PSTR("Main"), false);
                break;
            case _MAC_SYMBOLS:
                oled_write_bitmap_24x24(layer_symbols_bitmap, 4, 0, false);
                oled_set_cursor(1, 3);  // Center "Syms" (4 chars, so start at column 1)
                oled_write_P(PSTR("Syms"), false);
                break;
            case _MAC_UTIL:
                oled_write_bitmap_24x24(layer_util_bitmap, 4, 0, false);
                oled_set_cursor(1, 3);  // Center "Util" (4 chars, so start at column 1)
                oled_write_P(PSTR("Util"), false);
                break;
        }

        // WPM underneath the layer name
        oled_set_cursor(0, 4);
        sprintf(rgb_str, "WC:%03d", get_current_wpm());
        oled_write(rgb_str, false);

        // RGB info with name (move to last row or use remaining space)
        oled_set_cursor(0, 5);
        ...
        // Slave side - Eye blinking animation using bitmaps
        ...
        if (current_time - last_frame_time >= GIF_FRAME_DURATION_MS) {
        ...
        switch(current_frame) {
            case 0:
                oled_write_raw_P((const char*)radar_frame1, sizeof(radar_frame1));
                break;
            case 1:
                oled_write_raw_P((const char*)radar_frame2, sizeof(radar_frame2));
                break;
            case 2:
                oled_write_raw_P((const char*)radar_frame3, sizeof(radar_frame3));
                break;
            case 3:
                oled_write_raw_P((const char*)radar_frame4, sizeof(radar_frame4));
                break;
        }
    }
