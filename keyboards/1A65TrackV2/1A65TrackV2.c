...
void pointing_device_task(void){
    ...
    if(scrolly_pressed) {
        ...
        // int8_t scaled_scroll_h = scroll_h / SCROLL_DIVIDER / 20;
	...
	else if(scrollx_pressed) {
		// accumulate scroll
        scroll_h += clamped_x;
        scroll_v += clamped_y;

        int8_t scaled_scroll_h = scroll_h / SCROLL_DIVIDER;
        int8_t scaled_scroll_v = scroll_v / SCROLL_DIVIDER / 20;
...
bool process_record_kb(uint16_t keycode, keyrecord_t *record) {
    ...
    switch (keycode) {
        ...
        case KC_SCROLLY:
            scrolly_pressed = record->event.pressed;
            return false;
		
		case KC_SCROLLX:
			scrollx_pressed = record->event.pressed;
			return false;
        ...
        case UCFEMAIL:
			if (record->event.pressed) {
				SEND_STRING("santiago.garcia@ucf.edu");
			}
			return false;
