...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  ...
  [_QWERTY] = LAYOUT(
    ...
    KC_LCTL,  KC_LGUI,  TD(ALT_OSRAISE),              KC_SPC,                                 KC_RALT,  MO(1),    KC_RCTL,  KC_LEFT,  KC_DOWN,  KC_RGHT,            KC_P0,    KC_PDOT,  KC_PENT),
...
void alt_finished (qk_tap_dance_state_t *state, void *user_data) {
  ...
  switch (alttap_state.state) {
    case SINGLE_TAP: set_oneshot_layer(_RAISE, ONESHOT_START); clear_oneshot_layer_state(ONESHOT_PRESSED); break;
    case SINGLE_HOLD: register_code(KC_LALT); break;
    case DOUBLE_TAP: set_oneshot_layer(_RAISE, ONESHOT_START); set_oneshot_layer(_RAISE, ONESHOT_PRESSED); break;
    case DOUBLE_HOLD: register_code(KC_LALT); layer_on(_RAISE); break;
  }
}
...
void alt_reset (qk_tap_dance_state_t *state, void *user_data) {
  switch (alttap_state.state) {
    ...
    case DOUBLE_HOLD: layer_off(_RAISE); unregister_code(KC_LALT); break;
  }
