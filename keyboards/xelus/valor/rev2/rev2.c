...
led_config_t g_led_config = {{// Key Matrix to LED Index
                             ...
                             {// LED Index to Physical Position
                              {224, 0}, {224, 9}, {224, 21}, {224, 33}, {224, 45}, {223, 51}, {206, 52}, {190, 52}, {172, 52}, {154, 57}, {136, 61}, {118, 63}, {101, 64}, {80, 60}, {64, 57},
                              {46, 53}, {28, 53}, {10, 52},  {3, 51},   {2, 45},   {2, 33},   {2, 20},   {2, 9},    {0, 0},    {9, 3},    {6, 14},   {4, 26},   {21, 27}

                             },
                             {// LED Index to Flag
                              2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 8, 8, 8, 8}};
...
void rgb_led_indicator_helper(bool enable_indicator, HSV color, bool host_indicator, bool override, uint8_t key, uint8_t led_min, uint8_t led_max) {
    if (enable_indicator) {
        RGB rgb_led = hsv_to_rgb((HSV){.h = color.h, .s = color.s, .v = color.v});
        if (host_indicator) {
            RGB_MATRIX_INDICATOR_SET_COLOR(key, rgb_led.r, rgb_led.g, rgb_led.b);
        } else {
            if (override) {
                RGB_MATRIX_INDICATOR_SET_COLOR(key, 0, 0, 0);
            }
        }
    }
}
...
bool rgb_matrix_indicators_advanced_kb(uint8_t led_min, uint8_t led_max) {
    rgb_led_indicator_helper(g_config.enable_caps_lock, g_config.caps_lock_indicator, host_keyboard_led_state().caps_lock, g_config.caps_override_bl, g_config.caps_lock_key, led_min, led_max);

    rgb_led_indicator_helper(g_config.enable_num_lock, g_config.num_lock_indicator, host_keyboard_led_state().num_lock, g_config.num_override_bl, g_config.num_lock_key, led_min, led_max);

    rgb_led_indicator_helper(g_config.enable_scroll_lock, g_config.scroll_lock_indicator, host_keyboard_led_state().scroll_lock, g_config.scroll_override_bl, g_config.scroll_lock_key, led_min, led_max);
    ...
    if (g_config.enable_layer_indicator) {
        ...
        switch (get_highest_layer(layer_state)) {
            ...
            default:
                // white
                RGB_MATRIX_INDICATOR_SET_COLOR(g_config.layer_indicator_key, 128, 128, 128);
                break;
        }
    }
    return false;
