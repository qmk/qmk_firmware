...
enum layer_names { _QWERTY, _SYMB, _NAV, _ADJUST, _MOUSE, _NUMB, _FNR };

const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {

    [_QWERTY] = LAYOUT_split_3x5_3_encoder(
        KC_Q, KC_W, KC_E, KC_R, KC_T, KC_Y, KC_U, KC_I, KC_O, KC_P, KC_A, KC_S,
        KC_D, KC_F, KC_G, KC_H, KC_J, KC_K, KC_L, KC_SCLN, KC_Z, KC_X, KC_C,
        KC_V, KC_B, KC_LBRACKET, KC_RBRACKET, KC_N, KC_M, KC_COMM, KC_DOT,
        KC_SLSH, KC_LGUI, KC_BSPC, KC_SPC, KC_SPC, KC_ENT, KC_RALT),
...
void oled_task_user(void) {
  if (is_keyboard_master()) {
    render_status();  // Renders the current keyboard state (layer, lock, caps,
                      // scroll, etc)
  } else {
