...
//    RSFT_T(KC_MPLY), _______, _______, _______, _______, _______, _______, _______, KC_MNXT, KC_VOLD, KC_VOLU, KC_MPLY
//),
[_RAISE] = LAYOUT_planck_grid(
...
//    RSFT_T(KC_MPLY), _______, _______, _______, _______, _______, _______, _______, _______,  _______, _______, _______
//)
[_ADJUST] = LAYOUT_planck_grid(
...
//        if ((keyboard_report->mods & MOD_BIT (MOD_MASK_CTRL)) || (keyboard_report->mods & MOD_BIT (KC_RSFT))) {
        if (get_mods() & MOD_BIT(KC_RSFT)) {
            uint8_t mod_state = get_mods(); // Store which mods are held
            del_mods(MOD_MASK_SHIFT);       // Ignore all CTRL keys
          // Shift pressed -> < (Shift+,)
            if (clockwise) {
...
//      if (keyboard_report->mods & MOD_BIT (MOD_MASK_CTRL)) {
        if (get_mods() & MOD_BIT(KC_RSFT)) {
            uint8_t mod_state = get_mods(); // Store which mods are held
            del_mods(MOD_MASK_SHIFT);       // Ignore all CTRL keys
          // Shift pressed -> < (Shift+,)
