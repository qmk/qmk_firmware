...
void td_ent_lgui_lctl_spotlight_emoji_finished(tap_dance_state_t *state, void *user_data) {
...
void td_ent_lgui_lctl_spotlight_emoji_reset(tap_dance_state_t *state, void *user_data) {
    switch (xtap_state.state) {
...
void td_ent_lctl_lalt_windows_emoji_finished(tap_dance_state_t *state, void *user_data) {
    xtap_state.state = cur_dance(state);
    switch (xtap_state.state) {
...
void td_ent_lctl_lalt_windows_emoji_reset(tap_dance_state_t *state, void *user_data) {
    switch (xtap_state.state) {
        case TD_SINGLE_TAP: unregister_code(KC_ENT); break;
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [1] = LAYOUT_ortho_3x10_3(
        ...
        TD(TD_LCTL_ESC_SPOTLIGHT_EMOJI),                            KC_TRNS,                                      KC_TRNS
    ),
    [2] = LAYOUT_ortho_3x10_3(
        ...
        TD(TD_LCTL_ESC_SPOTLIGHT_EMOJI),                           LT(1, KC_SPC),                            KC_TRNS
    ),
    ...
    [5] = LAYOUT_ortho_3x10_3(
        ...
        KC_LCBR,                       KC_LPRN, KC_RPRN, KC_RCBR, KC_PIPE, KC_ESC,   KC_LEFT, KC_UP,   KC_DOWN, KC_RGHT,
        LSFT_T(KC_LBRC),               KC_QUOT, KC_DQUO, KC_RBRC, KC_SCLN, KC_TILDE, KC_VOLD, KC_MUTE, KC_VOLU, RSFT_T(KC_BSLS),
        TD(TD_LALT_ESC_WINDOWS_EMOJI),                            KC_TRNS,                                      KC_TRNS
    ),
    [6] = LAYOUT_ortho_3x10_3(
        KC_EXLM, KC_AT,                KC_HASH, KC_DLR, KC_PERC, KC_CIRC,       KC_AMPR, KC_ASTR, KC_CAPS, KC_BSPC,
        KC_1,    KC_2,                 KC_3,    KC_4,   KC_5,    KC_6,          KC_7,    KC_8,    KC_9,    KC_0,
        KC_LSFT, KC_NO,                KC_NO,   KC_NO,  MO(7),   KC_NO,         KC_NO,   KC_COMM, KC_DOT,  RSFT_T(KC_SLSH),
        TD(TD_LALT_ESC_WINDOWS_EMOJI),                           LT(5, KC_SPC),                            KC_TRNS
    ),
    [7] = LAYOUT_ortho_3x10_3(
