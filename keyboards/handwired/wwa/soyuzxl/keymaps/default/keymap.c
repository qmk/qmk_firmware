...
void slider(void) {
    ...
    int currentPotValue4 = analogReadPin(POT4_PIN);


    if (abs(currentPotValue4 - previousPotValue4) > threshold) {
        midi_send_cc(&midi_device, 5, 0x0D, 0x7F - (currentPotValue4 >> 3));
        previousPotValue4 = currentPotValue4;
    }

    static int previousPotValue5 = 0;
    int currentPotValue5 = analogReadPin(POT5_PIN);


    if (abs(currentPotValue5 - previousPotValue5) > threshold) {
        midi_send_cc(&midi_device, 6, 0x0E, 0x7F - (currentPotValue5 >> 3));
        previousPotValue5 = currentPotValue5;
    }
