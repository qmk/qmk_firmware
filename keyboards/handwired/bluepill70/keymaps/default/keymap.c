...
const uint16_t keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  ...
  KC_TAB,  KC_Q,    KC_W,    KC_E,    KC_R,    KC_T,    KC_Y,    KC_U,    KC_I,    KC_O,    KC_P,    KC_LBRC, KC_RBRC,          KC_SLASH,KC_PGUP,   \
  KC_CAPS, KC_A,    KC_S,    KC_D,    KC_F,    KC_G,    KC_H,    KC_J,    KC_K,    KC_L,    KC_SCLN, KC_QUOT,                   KC_ENT,  KC_PGDOWN, \
  KC_LSPO,          KC_Z,    KC_X,    KC_C,    KC_V,    KC_B,    KC_N,    KC_M,    KC_COMM, KC_DOT,  KC_SLSH, KC_RSPC,          KC_UP,   TT(2),     \
  KC_LCTL, KC_LGUI, KC_LALT,                        KC_SPC,                                 KC_RALT, MO(1),   KC_RCTL, KC_LEFT, KC_DOWN, KC_RIGHT ),
        
  [_FNONE] = LAYOUT_ansi(
  _______, KC_MPRV, KC_MSTP, KC_MPLY, KC_MNXT, _______, KC_MUTE, KC_VOLD, KC_VOLU, _______, _______, KC_NLCK, KC_CALC, _______, _______, _______, \
  ...
  _______,          _______, _______, _______, _______, _______, _______, KC_P0,   KC_PDOT, KC_PENT, KC_PSLS, _______,          _______, _______, \
...
uint32_t layer_state_set_user(uint32_t state) {
  ...
  case _FNONE:
    writePinHigh(A0); //OFF Color A
    writePinLow(A1);  //ON Color B
    break;
  case _FNTWO:
    writePinLow(A0); //ON Color A
    writePinLow(A1); //ON Color B
    break;
  default: //  for any other layers, or the default layer
    writePinLow(A0);  //ON Color A
    writePinHigh(A1); //OFF Color B
    break;
  }
  return state;
}
