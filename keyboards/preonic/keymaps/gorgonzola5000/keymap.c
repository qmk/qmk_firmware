...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
	mod_state = get_mods();
	switch(keycode){
		case RELEASE: // disable OSMs and switch to default layer
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
	[_DEFAULT] = LAYOUT_preonic_1x2uC(							
		...
		RELEASE,		OSM(MOD_LCTL), 	OSM(MOD_LGUI), 	OSM(MOD_LALT), 	OSM(MOD_LSFT), 			KC_SPC,					OSM(MOD_RALT), 	OSL(_ARROWS),	OSL(_F_KEYS), 	KC_F13,			KC_DEL
		),
