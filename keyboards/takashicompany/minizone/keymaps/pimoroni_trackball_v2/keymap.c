...
report_mouse_t pointing_device_task_user(report_mouse_t mouse_report) {
    ...
    if (current_state == POINTER_INPUT)
    {
        ...
        {
            counter = 0;
            int16_t absX = my_abs(record_x);
            int16_t absY = my_abs(record_y);
            isXMin = absX < absY;

            // move_count = isXMin ? absY : absX;
            move_count = absY + absX;
            move_count *= 5;

            x_sign = my_sign(record_x);
            y_sign = my_sign(record_y);
    ...
    if (move_count > 0)
    {
        ...
        if (isXMin)
        {
            mouse_report.y = y_sign;

            if (minMove)
            {
                mouse_report.x = x_sign;
            }
        }
        else
        {
