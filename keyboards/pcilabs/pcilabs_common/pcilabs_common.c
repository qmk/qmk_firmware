...
void raw_hid_receive(uint8_t *data, uint8_t length) {
    switch (data[0]) {
        case 2:;
            /* new report structure: id, row_num, row_low, row_high, switch_type, mode, actuation_point_low, actuation_point_high, press_sensitivity, release_sensitivity, press_hysteresis, release_hysteresis */
            uint8_t      row        = data[1];
            uint16_t     row_val    = (uint16_t)data[2] | (uint16_t)data[3] << 8;
            key_config_t new_config = {.switch_type = data[4], .mode = data[5], .actuation_point = (uint16_t)data[6] | (uint16_t)data[7] << 8, .press_sensitivity = data[8], .release_sensitivity = data[9], .press_hysteresis = data[10], .release_hysteresis = data[11]};
            for (uint8_t col = 0; col < MATRIX_COLS; col++) {
                if ((row_val >> col) & 1) {
