...
void eeconfig_update_rgblight_default(void) {
    ...
    rgblight_config.mode   = RGBLIGHT_MODE_RAINBOW_SWIRL;  //lizm modified 出厂灯条动画
    rgblight_config.hue    = 0;
    rgblight_config.sat    = UINT8_MAX;
    rgblight_config.val    = RGBLIGHT_LIMIT_VAL/2;   //lizm modified,出厂亮度
...
#    endif
    ...
    if(lizm_restart_snake){
        if (increment == 1) {
            pos = rgblight_ranges.effect_num_leds - 4; //解决闪一下的问题，从第四个灯开始
        } else {
            pos = 3;  //解决闪一下的问题，从第四个灯开始
        }
        anim->pos = 1;
        lizm_restart_snake = false;
