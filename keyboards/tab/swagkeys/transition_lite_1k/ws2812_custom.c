...
// WS2812_LED_COUNT RGBLIGHT_LED_COUNT
...
#if WS2812_CONCAT(WS2812_PWM_TIMER_32BIT_, WS2812_RGBLIGHT_PWM_DRIVER)
#    define WS2812_PWM_TIMER_32BIT
#endif

#ifndef WS2812_RGBLIGHT_PWM_COMPLEMENTARY_OUTPUT
...
#    else
#        define WS2812_OUTPUT_MODE PAL_MODE_ALTERNATE(WS2812_RGBLIGHT_PWM_PAL_MODE) | PAL_OUTPUT_TYPE_PUSHPULL | PAL_OUTPUT_SPEED_HIGHEST | PAL_PUPDR_FLOATING
#    endif
#else
#    if defined(USE_GPIOV1)
#        define WS2812_OUTPUT_MODE PAL_MODE_ALTERNATE_OPENDRAIN
#    else
#        define WS2812_OUTPUT_MODE PAL_MODE_ALTERNATE(WS2812_RGBLIGHT_PWM_PAL_MODE) | PAL_OUTPUT_TYPE_OPENDRAIN | PAL_OUTPUT_SPEED_HIGHEST | PAL_PUPDR_FLOATING
#    endif
#endif
...
#define WS2812_COLOR_BIT_N (RGBLIGHT_LED_COUNT * WS2812_COLOR_BITS) /**< Number of data bits */
 ...
 *
 * @param[in] led:                  The led index [0, @ref RGBLIGHT_LED_COUNT)
...
void ws2812_rgblight_init(void) {
    ...
    palSetLineMode(WS2812_RGBLIGHT_DI_PIN, WS2812_OUTPUT_MODE);
        ...
        .channels =
            {
                ...
                [WS2812_RGBLIGHT_PWM_CHANNEL - 1] = {.mode = WS2812_PWM_OUTPUT_MODE, .callback = NULL}, // Turn on the channel we care about
            },
        .cr2  = 0,
    ...
    dmaStreamAlloc(WS2812_RGBLIGHT_DMA_STREAM - WB32_DMA_STREAM(0), 10, NULL, NULL);
    dmaStreamSetSource(WS2812_RGBLIGHT_DMA_STREAM, ws2812_frame_buffer);
    dmaStreamSetDestination(WS2812_RGBLIGHT_DMA_STREAM, &(WS2812_RGBLIGHT_PWM_DRIVER.tim->CCR[WS2812_RGBLIGHT_PWM_CHANNEL - 1])); // Ziel ist der An-Zeit im Cap-Comp-Register
    dmaStreamSetMode(WS2812_RGBLIGHT_DMA_STREAM, WB32_DMA_CHCFG_HWHIF(WS2812_RGBLIGHT_DMA_CHANNEL) | WB32_DMA_CHCFG_DIR_M2P | WB32_DMA_CHCFG_PSIZE_WORD | WB32_DMA_CHCFG_MSIZE_WORD | WB32_DMA_CHCFG_MINC | WB32_DMA_CHCFG_CIRC | WB32_DMA_CHCFG_TCIE | WB32_DMA_CHCFG_PL(3));
#else
    dmaStreamAlloc(WS2812_RGBLIGHT_DMA_STREAM - STM32_DMA_STREAM(0), 10, NULL, NULL);
    dmaStreamSetPeripheral(WS2812_RGBLIGHT_DMA_STREAM, &(WS2812_RGBLIGHT_PWM_DRIVER.tim->CCR[WS2812_RGBLIGHT_PWM_CHANNEL - 1])); // Ziel ist der An-Zeit im Cap-Comp-Register
    dmaStreamSetMemory0(WS2812_RGBLIGHT_DMA_STREAM, ws2812_frame_buffer);
    dmaStreamSetMode(WS2812_RGBLIGHT_DMA_STREAM, STM32_DMA_CR_CHSEL(WS2812_RGBLIGHT_DMA_CHANNEL) | STM32_DMA_CR_DIR_M2P | WS2812_DMA_PERIPHERAL_WIDTH | WS2812_DMA_MEMORY_WIDTH | STM32_DMA_CR_MINC | STM32_DMA_CR_CIRC | STM32_DMA_CR_PL(3));
#endif
    dmaStreamSetTransactionSize(WS2812_RGBLIGHT_DMA_STREAM, WS2812_BIT_N);
    // M2P: Memory 2 Periph; PL: Priority Level

#if (STM32_DMA_SUPPORTS_DMAMUX == TRUE)
    // If the MCU has a DMAMUX we need to assign the correct resource
    dmaSetRequestSource(WS2812_RGBLIGHT_DMA_STREAM, WS2812_DMAMUX_ID);
#endif

    // Start DMA
    dmaStreamEnable(WS2812_RGBLIGHT_DMA_STREAM);

    // Configure PWM
    // NOTE: It's required that preload be enabled on the timer channel CCR register. This is currently enabled in the
    // ChibiOS driver code, so we don't have to do anything special to the timer. If we did, we'd have to start the timer,
    // disable counting, enable the channel, and then make whatever configuration changes we need.
    pwmStart(&WS2812_RGBLIGHT_PWM_DRIVER, &ws2812_pwm_config);
    pwmEnableChannel(&WS2812_RGBLIGHT_PWM_DRIVER, WS2812_RGBLIGHT_PWM_CHANNEL - 1, 0); // Initial period is 0; output will be low until first duty cycle is DMA'd in
}

void ws2812_rgblight_write_led(uint16_t led_number, uint8_t r, uint8_t g, uint8_t b) {
...
void ws2812_rgblight_write_led_rgbw(uint16_t led_number, uint8_t r, uint8_t g, uint8_t b, uint8_t w) {
    // Write color to frame buffer
    for (uint8_t bit = 0; bit < 8; bit++) {
        ws2812_frame_buffer[WS2812_RED_BIT(led_number, bit)]   = ((r >> bit) & 0x01) ? WS2812_DUTYCYCLE_1 : WS2812_DUTYCYCLE_0;
        ws2812_frame_buffer[WS2812_GREEN_BIT(led_number, bit)] = ((g >> bit) & 0x01) ? WS2812_DUTYCYCLE_1 : WS2812_DUTYCYCLE_0;
...
void ws2812_rgblight_setleds(rgb_led_t* ledarray, uint16_t leds) {
    static bool s_init = false;
    if (!s_init) {
        ws2812_rgblight_init();
        s_init = true;
    }

    for (uint16_t i = 0; i < leds; i++) {
        ws2812_rgblight_write_led(i, ledarray[i].r, ledarray[i].g, ledarray[i].b);
    }

    // for (uint16_t i = 0; i < 26; i++) {
    //     ws2812_rgblight_write_led(i, 0x11, 0x11, 0x11);
    // }
}
