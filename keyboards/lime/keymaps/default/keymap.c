...
static void print_status_narrow(void) {
    ...
    /* Joystick debugging */
    oled_set_cursor(0,9);
    uint16_t x_val = analogReadPin(F5);
    char x_val_str[5];
    itoa(x_val, x_val_str, 10);
    oled_write(x_val_str, false);

    oled_set_cursor(0,10);
    uint16_t y_val = analogReadPin(F4);
    char y_val_str[5];
...
#   endif
    ...
    joystick_config_t joystick_axes[JOYSTICK_AXES_COUNT] = {
        [0] = JOYSTICK_AXIS_IN(JOYSTICK_X_PIN, 268, 562, 813),
        [1] = JOYSTICK_AXIS_IN(JOYSTICK_Y_PIN, 865, 571, 260)
    };
