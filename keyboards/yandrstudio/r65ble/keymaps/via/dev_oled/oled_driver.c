...
void oled_set_precise_cursor(uint16_t index) {
    // Out of bounds?
    if (index >= OLED_MATRIX_SIZE) {
        index = 0;
    }
...
void oled_advance_page(bool clearPageRemainder) {
    ...
    uint8_t  remaining = (oled_rotation_width - (index % oled_rotation_width))%oled_rotation_width;
...
void oled_precise_pan(bool left, uint16_t index) {
    if (index>OLED_MATRIX_SIZE-1) {
        return;
    }
    if (left) {
        for (uint16_t x = 0; x < index; x++) {
            oled_buffer[x] = oled_buffer[x + 1];
        }
    } else {
        for (uint16_t x = OLED_MATRIX_SIZE - 1; x > index; x--) {
            oled_buffer[x] = oled_buffer[x - 1];
        }
    }
    oled_dirty = OLED_ALL_BLOCKS_MASK;
}
void oled_precise_pan_h(bool left, uint16_t start_line) {
    uint16_t i = 0;
    uint16_t oled_rotation_height = 8 * OLED_MATRIX_SIZE / oled_rotation_width;
    for (uint16_t y = start_line; y < oled_rotation_height / 8; y++) {
        if (left) {
            for (uint16_t x = 0; x < oled_rotation_width - 1; x++) {
                i              = y * oled_rotation_width + x;
                oled_buffer[i] = oled_buffer[i + 1];
            }
        } else {
            for (uint16_t x = oled_rotation_width - 1; x > 0; x--) {
                i              = y * oled_rotation_width + x;
                oled_buffer[i] = oled_buffer[i - 1];
            }
        }
...
void oled_write_raw(const char *data, uint16_t size) {
    ...
    for (uint16_t j = 0, i = cursor_start_index; i < cursor_start_index + size; i++, j++) {
        if (oled_buffer[i] == data[j]) continue;
        oled_buffer[i] = data[j];
