...
static void select_mux_channel(uint8_t channel) {
    channel &= 0x07;
    writePin(MUX_PIN_A, channel & 0x01);
    writePin(MUX_PIN_B, channel & 0x02);
    writePin(MUX_PIN_C, channel & 0x04);
    wait_us(2);
}

static void enable_direct_row(void) {
    setPinOutput(DIRECT_ROW_PIN);
    writePinLow(DIRECT_ROW_PIN);
    wait_us(2);
}

static void disable_direct_row(void) {
    writePinHigh(DIRECT_ROW_PIN);
    setPinInputHigh(DIRECT_ROW_PIN);
}

void matrix_init_custom(void) {
    setPinOutput(MUX_PIN_A);
    setPinOutput(MUX_PIN_B);
    setPinOutput(MUX_PIN_C);
    setPinOutput(MUX_PIN_COM);
