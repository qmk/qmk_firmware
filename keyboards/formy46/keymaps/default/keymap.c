...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [_BASE] = LAYOUT(
        ...
        KC_Z,    KC_X,    KC_C,    KC_V,    KC_B,        JS_MODE,                       KC_GRV,      KC_N,    KC_M,    KC_COMM, KC_DOT,  KC_SLSH,
        KC_LSFT, KC_LCTL, KC_LGUI, KC_LALT,     KC_NO,   KC_SPC,  RE_MODE,     KC_BSPC, KC_ENT,  KC_DEL,      KC_RALT, KC_RGUI, KC_RCTL, FUNC
    ),
    /* Game Mode */
    [_GAME_MODE] = LAYOUT(
...
bool encoder_update_user(uint8_t index, bool clockwise) {
    switch(get_highest_layer(layer_state)){
        case _BASE:
            if (re_function){
                if (clockwise) {
                    tap_code16(DF(_GAME_MODE));
                } else {
                    tap_code16(DF(_CREATION_MODE));
                }
            } else {
                if (clockwise) {
                    tap_code(KC_WH_U);
