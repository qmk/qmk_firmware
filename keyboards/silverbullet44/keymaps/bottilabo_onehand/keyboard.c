...
uint8_t inline LEDTR(uint8_t i) {
    return lttbl[ i%sizeof(lttbl) ];
}
...
void matrix_scan_local(void) { 
    if( update_led_layer ) {
        ...
        if( a[fct1] == 0 ) {
           ...
           setrgb(r, g, b, (LED_TYPE *)&led[ LEDTR(i)]);
        }
    }
...
bool process_record_local(uint16_t keycode, keyrecord_t *record) {
    ...
    {
        //char buf[10];
        //sprintf(buf,"%d,%d ",record->event.key.col,record->event.key.row);
        //send_string(buf);
