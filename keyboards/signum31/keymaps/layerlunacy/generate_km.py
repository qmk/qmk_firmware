...
def toLgd(s):
    if re.match("^[0-9a-zA-Z]$", s):
        return "   {0}   ".format(s)
    if re.match("^[FLT][0-9]{1,2}$", s): # Fn/Ln/Tn, works from F0 to F99, L0 to L99, T0 to T99
        return "{0:^7}".format(s)
    elif s in layout.uc_dict:
        return "   {0}   ".format(chr(int(layout.uc_dict[s], 0)))
    elif s in layout.qmk_dict:
        return "{0:^7}".format(s)
    elif s == s.upper() and s.startswith("KC_"):
        return "{0:^7}".format(s[2:])
    else:
        return "  {0}   ".format(chr(128165))
...
#            output.write(str(layout.uc_dict))
        ...
        if line.startswith("<keymaps/>"):
            ...
            for L in layout.layers:
                ...
                output.write(" * -----------------------------------------------------------------------------------------------------------------\n")
                output.write(" *                                 |{0}|{1}|{2}|{3}|{4}|{5}|\n".format(toLgd(L[4][0]), toLgd(L[4][1]), toLgd(L[4][2]), toLgd(L[4][3]), toLgd(L[4][4]), toLgd(L[4][5])))
                output.write(" *                                 -------------------------------------------------\n")
                output.write(" */\n")
                output.write("\tKEYMAP(\n")
                ...
                output.write("\t\tXXXXXXX, {0}, XXXXXXX, XXXXXXX, {1}, {2}, XXXXXXX, XXXXXXX, {3}, XXXXXXX,\n".format(toKC(L[4][2]), toKC(L[4][0]), toKC(L[4][5]), toKC(L[4][3])))
