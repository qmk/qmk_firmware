...
int usb7206_read_reg(struct USB7206* self, uint32_t addr, uint8_t* data, int length) {
    ...
    uint8_t command[9] = {
        0x00,                   // Buffer address high: always 0
        0x00,                   // Buffer address low: always 0
    ...
    uint8_t command2[2] = {
        0x00,  // Buffer address high: always 0
        0x06,  // Buffer address low: 6 to skip header
    };
...
int usb7206_write_reg(struct USB7206* self, uint32_t addr, uint8_t* data, int length) {
    ...
    uint8_t command[9] = {
        0x00,                   // Buffer address high: always 0
        0x00,                   // Buffer address low: always 0
        ((uint8_t)length) + 6,  // Number of bytes to write to command block buffer area (TODO: check length!)
...
// Returns number of bytes written on success or a negative number on error.
int usb7206_write_reg_8(struct USB7206* self, uint32_t addr, uint8_t data) { return usb7206_write_reg(self, addr, &data, sizeof(data)); }
...
int ptn5110_get_cc_status(struct PTN5110* self, uint8_t* cc) { return i2c_get(self->addr, 0x1D, cc, 1); }
