...
I'm attempting to solve this by some creative use of QMK's macro feature. The keycode in the System layer that enables these layers, [`GO_Q2`](./keymap.c#L198), is a [macro](../../../../users/noroadsleft/noroadsleft.h#L129-L137) that sets the default layer to the QWERTY layer, then turns the Quake 2 layer `_Q2` on. The result is a partially-overwritten QWERTY layer, that has some keycodes with some creative layer switching.

When I hit the `Enter` key (bound in-game to text chat), the [macro keycode](../../../../users/noroadsleft/noroadsleft.h#L138-L145) I've created sends the keycode for `Enter`, then follows with enabling the Hardware Dvorak layer and its corresponding overlay. Now the game is in text chat mode, and my keyboard is in Dvorak. When I hit `Enter` again, another `Enter` [keycode macro](../../../../users/noroadsleft/noroadsleft.h#L146-L153) is sent, which sends the message, then the macro brings me back to the standard QWERTY+Quake 2 setup. Hitting `Escape` instead runs a [macro](../../../../users/noroadsleft/noroadsleft.h#L154-L161) that cancels the sending of the message, and undoes the layers.
