...
void dial_sw_scan(void)
{
    ...
    // System switch: still read hardware state for indication,
    // but use a single base layer (layer 0). NKRO follows hardware switch.
    if (dial_scan & 0x02) {
...
void m_power_on_dial_sw_scan(void)
{
    ...
    // Win or Mac - single base layer, just track state
    if (dial_scan_sys) {
        if (dev_info.sys_sw_state != SYS_SW_WIN) {
            default_layer_set(1 << 0);
            dev_info.sys_sw_state = SYS_SW_WIN;
...
{
    nuphy_eeconfig_read_user(&user_config);
