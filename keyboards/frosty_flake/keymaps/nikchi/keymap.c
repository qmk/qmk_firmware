...
void matrix_scan_user(void) {
  LEADER_DICTIONARY() {
    ...
    SEQ_THREE_KEYS(KC_L,KC_I,KC_T) { // 🔥🔥 Leader key then type lit
      unicode_input_start();
...
void cycleAll(qk_tap_dance_state_t *state, void *user_data) {
  if(state->count == 1) {
    unicode_input_start();
    register_hex32(pgm_read_dword(&unicode_map[2]));
    unicode_input_finish();
  }
  else if(state->count <= 1642) {
    tap(KC_BSPC);
    unicode_input_start();
    register_hex32(pgm_read_dword(&unicode_map[2])+state->count);
    unicode_input_finish();
  }
};
