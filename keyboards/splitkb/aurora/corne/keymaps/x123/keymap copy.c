...
void on_smtd_action(uint16_t keycode, smtd_action action, uint8_t tap_count) {
    switch (keycode) {
        ...
        SMTD_MT(CKC_SNAPS, KC_ENT, BUILD_SNAPS)
...
// bool snap_tap_handler(uint16_t keycode, keyrecord_t *record)
// {
//     SNAP_TAP_T *entry = find_snap_tap_entry(snap_tap_entries_head, keycode);
//     bool default_key = true;
    
//     if (entry != NULL)
//     {
//         SNAP_TAP_T *opposing_entry = find_snap_tap_entry(snap_tap_entries_head, entry->opposing_keycode);
...
bool process_record_user(uint16_t keycode, keyrecord_t *record)
{
    ...
    if (get_highest_layer(layer_state | default_layer_state) == _GAMER && 
        !snap_tap_wrapper(snap_tap_entries_head, keycode, record))
    {
        return false;
    }
