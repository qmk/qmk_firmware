...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
	...
	KEYMAP(
		...
		KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, BL_DEC, BL_TOGG, BL_INC, BL_STEP, KC_TRNS, KC_TRNS, M(0), M(1), KC_TRNS, KC_TRNS, KC_TRNS, 
	...
	switch (id) {
		case 0:
			if (record->event.pressed) {
				return MACRO( T(C), T(H), T(E), T(S), T(T), T(E), T(R), T(C), T(R), T(I), T(S), T(T), T(O), T(B), T(A), T(L), D(LSFT), T(2), U(LSFT), T(G), T(M), T(A), T(I), T(L), T(DOT), T(C), T(O), T(M), END );
			}
			break;
		case 1:
			if (record->event.pressed) {
				return MACRO( T(I), T(C), T(O), T(N), T(S), T(T), T(E), T(L), T(L), T(A), T(T), T(I), T(O), T(N), T(I), D(LSFT), T(2), U(LSFT), T(G), T(M), T(A), T(I), T(L), T(DOT), T(C), T(O), T(M), END );
			}
			break;
	}
	return MACRO_NONE;
