/*
 * some macros to help with tap dance.  usage:
 * call this in your keymap
 *     TD_TAP_HOLD_FUNCTIONS(name, key code on tap, key code on hold)
 * then in the tap_dance_actions array add
 *     [ID] = TD_TAP_HOLD_ACTION(name)
 * now your keymap can reference TD(ID)  [ID should probably be in an enum]
 */
typedef struct {
