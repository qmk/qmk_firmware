...
#define KC_CTLEN RCTL_T(KC_ENT)
#define KC_LALT LALT_T(KC_SPC)
#define KC_RALT RALT_T(KC_SPC)
#define KC_LSPA LT(_LOWER, KC_SPC)
#define KC_RSPA LT(_RAISE, KC_SPC)
#define KC_AMUTE KC__MUTE
#define KC_VDOWN KC__VOLDOWN
...
// Setting MEDIA layer RGB back to default
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
  ...
  switch (keycode) {
    ...
    case LOWER:
      if (record->event.pressed) {
        layer_on(_LOWER);
        update_tri_layer_RGB(_LOWER, _RAISE, _MEDIA);
      } else {
        layer_off(_LOWER);
        update_tri_layer_RGB(_LOWER, _RAISE, _MEDIA);
      }
