...
static void oled_render_anim_frame(const char **fast_frames, const char **slow_frames, uint8_t frames_len) {
  ...
  const uint32_t idle_threshold = 60000;  // 60 seconds

  // If typing activity is detected, reset the idle timer
  // if (speed > 0) {
  //   idle_timer = timer_read32();  // Reset idle timer on typing
  //   oled_on();
  // }

   // Check if idle time has exceeded the threshold
  if (timer_elapsed32(idle_timer) > idle_threshold) {
    // Stop the animation (do nothing and return early)
    oled_off();
    return;
  }
