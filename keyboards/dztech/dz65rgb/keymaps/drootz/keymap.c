...
{
	recording_r = get_blink_rgb(h, s, v, 0); // R
	recording_g = get_blink_rgb(h, s, v, 1); // G
	recording_b = get_blink_rgb(h, s, v, 2); // B
}
...
// Mac require some time before Capslock register in macros
...
bool process_record_user(uint16_t keycode, keyrecord_t *record)
{
	...
	if (pressed) {
		switch (keycode) {
			case TO(_MAC):
				// change global layer indicator bool
				onMac = true;
				break;
			case TO(_MAIN):
				// change global layer indicator bool
...
void matrix_scan_user(void) 
{
	LEADER_DICTIONARY() 
	{
		...
		// _MAIN & _MAC
	    //	éÉ		=> LdrKey > /
	    SEQ_ONE_KEY(KC_SLSH) { send_e_aigu(); }
	    //	éÉ		=> LdrKey > E > E > E
	    SEQ_THREE_KEYS(KC_E, KC_E, KC_E) { send_e_aigu(); }
	    //	àÀ		=> LdrKey > ; > A
	    SEQ_TWO_KEYS(KC_SCLN, KC_A) { lk_send_a_grave(); }
	    //	àÀ		=> LdrKey > A
	    SEQ_ONE_KEY(KC_A) { lk_send_a_grave(); }
	    //	èÈ		=> LdrKey > ; > E
	    SEQ_TWO_KEYS(KC_SCLN, KC_E) { lk_send_e_grave(); }
	    //	èÈ		=> LdrKey > E
	    SEQ_ONE_KEY(KC_E) { lk_send_e_grave(); }
	    //	ùÙ		=> LdrKey > ; > U
	    SEQ_TWO_KEYS(KC_SCLN, KC_U) { lk_send_u_grave(); }
	    //	ùÙ		=> LdrKey > U
	    SEQ_ONE_KEY(KC_U) { lk_send_u_grave(); }
	    //	âÂ		=> LdrKey > [ > A
	    SEQ_TWO_KEYS(KC_LBRC, KC_A) { lk_send_a_circonflexe(); }
	    //	âÂ		=> LdrKey > A > A
	    SEQ_TWO_KEYS(KC_A, KC_A) { lk_send_a_circonflexe(); }
	    //	êÊ		=> LdrKey > [ > E
	    SEQ_TWO_KEYS(KC_LBRC, KC_E) { lk_send_e_circonflexe(); }
	    //	êÊ		=> LdrKey > E > E
	    SEQ_TWO_KEYS(KC_E, KC_E) { lk_send_e_circonflexe(); }
	    //	îÎ		=> LdrKey > [ > I
	    SEQ_TWO_KEYS(KC_LBRC, KC_I) { lk_send_i_circonflexe(); }
	    //	îÎ		=> LdrKey > I > I
	    SEQ_TWO_KEYS(KC_I, KC_I) { lk_send_i_circonflexe(); }
	    //	ôÔ		=> LdrKey > [ > O
	    SEQ_TWO_KEYS(KC_LBRC, KC_O) { lk_send_o_circonflexe(); }
	    //	ôÔ		=> LdrKey > O > O
	    SEQ_TWO_KEYS(KC_O, KC_O) { lk_send_o_circonflexe(); }
	    //	ûÛ		=> LdrKey > [ > U
	    SEQ_TWO_KEYS(KC_LBRC, KC_U) { lk_send_u_circonflexe(); }
	    //	ûÛ		=> LdrKey > U > U
	    SEQ_TWO_KEYS(KC_U, KC_U) { lk_send_u_circonflexe(); }
	    //	çÇ		=> LdrKey > ] > C
	    SEQ_TWO_KEYS(KC_RBRC, KC_C) { lk_send_c_cedille(); }
	    //	çÇ		=> LdrKey > C
	    SEQ_ONE_KEY(KC_C) { lk_send_c_cedille(); }
	    //	CapsLock
	    SEQ_ONE_KEY(KC_LEAD) { registerCapsLock(); }
		...
		// // HELPER => PRINT/DEBUG CURRENT LAYER
		// SEQ_FIVE_KEYS(KC_INS, KC_INS, KC_INS, KC_INS, KC_INS) { 
		// 	onMac ? SEND_STRING("Layer: on _MAC\n") : SEND_STRING("Layer: on _MAIN\n"); 
		// 	layer_state_is(1) ? SEND_STRING("Layer 1\n") : SEND_STRING("Layer 0\n");
		// } 
	}
}

void leader_start(void) { isLeader = true; }
void leader_end(void) { isLeader = false; }
...
{
	onMac = false; // reset layer as dynamic macro clear the keyboard and reset layers.
