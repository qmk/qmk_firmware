...
static uint8_t _eeprom_read_byte(const uint8_t *addr) {
    ...
    if (eeprom_is_busy()) {
        eeprom_printf("eeprom_read_byte:: timeout\n");
        return 0x0;
    }
...
static void _eeprom_write_byte(uint8_t *addr, uint8_t value) {
    uintptr_t offset = (uintptr_t)addr;
    if (offset >= MAX(EEPROM_SIZE, get_virtual_eeprom_size())) {
...
void eeprom_driver_erase(void) {
    size_t   len = MAX(EEPROM_SIZE, get_virtual_eeprom_size());
    uint8_t *p   = (uint8_t *)0;
    while (len--) {
        _eeprom_write_byte(p++, 0);
    }
}
