...
void snled27351_write_pwm_buffer(uint8_t addr, uint8_t index) {
    ...
    for (uint8_t i = 0; i < SNLED27351_PWM_REGISTER_COUNT; i += 16) {
        ...
        for (uint8_t j = 0; j < SNLED27351_I2C_PERSISTENCE; j++) {
            if (i2c_write_register(addr << 1, i, driver_buffers[index].pwm_buffer + i, 16, SNLED27351_I2C_TIMEOUT) == I2C_STATUS_SUCCESS) break;
        }
#else
        i2c_write_register(addr << 1, i, driver_buffers[index].pwm_buffer + i, 16, SNLED27351_I2C_TIMEOUT);
#endif
    }
}
...
void snled27351_update_led_control_registers(uint8_t addr, uint8_t index) {
    if (driver_buffers[index].led_control_buffer_dirty) {
        ...
        for (uint8_t i = 0; i < SNLED27351_LED_CONTROL_REGISTER_COUNT; i++) {
            snled27351_write_register(addr, i, driver_buffers[index].led_control_buffer[i]);
        }
