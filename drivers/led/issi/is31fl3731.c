...
#ifdef IS31FL3731_DOUBLE_BUFFER
    uint8_t pwm_flush_buffer[IS31FL3731_PWM_REGISTER_COUNT];
#endif
...
#    ifdef IS31FL3731_DOUBLE_BUFFER
            if (i2c_write_register(i2c_addresses[index] << 1, IS31FL3731_FRAME_REG_PWM + i, driver_buffers[index].pwm_flush_buffer + i, 16, IS31FL3731_I2C_TIMEOUT) == I2C_STATUS_SUCCESS) break;
#    else
...
#else
#    ifdef IS31FL3731_DOUBLE_BUFFER
        i2c_write_register(i2c_addresses[index] << 1, IS31FL3731_FRAME_REG_PWM + i, driver_buffers[index].pwm_flush_buffer + i, 16, IS31FL3731_I2C_TIMEOUT);
#    else
...
#ifdef IS31FL3731_DOUBLE_BUFFER
        if (memcmp(driver_buffers[index].pwm_buffer, driver_buffers[index].pwm_flush_buffer, IS31FL3731_PWM_REGISTER_COUNT) == 0) {
            // if they are the same return early
            return;
        }

        // copy the current buffer to the flush buffer
        memcpy(driver_buffers[index].pwm_flush_buffer, driver_buffers[index].pwm_buffer, IS31FL3731_PWM_REGISTER_COUNT);
#endif
