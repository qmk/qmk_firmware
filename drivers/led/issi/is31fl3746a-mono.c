...
#ifdef IS31FL3746A_DOUBLE_BUFFER
    uint8_t pwm_flush_buffer[IS31FL3746A_PWM_REGISTER_COUNT];
#endif
...
#    ifdef IS31FL3746A_DOUBLE_BUFFER
            if (i2c_write_register(i2c_addresses[index] << 1, i, driver_buffers[index].pwm_flush_buffer + i, 18, IS31FL3746A_I2C_TIMEOUT) == I2C_STATUS_SUCCESS) break;
#    else
            if (i2c_write_register(i2c_addresses[index] << 1, i, driver_buffers[index].pwm_buffer + i, 18, IS31FL3746A_I2C_TIMEOUT) == I2C_STATUS_SUCCESS) break;
#    endif
        }
#else
#    ifdef IS31FL3746A_DOUBLE_BUFFER
        i2c_write_register(i2c_addresses[index] << 1, i, driver_buffers[index].pwm_flush_buffer + i, 18, IS31FL3746A_I2C_TIMEOUT);
#    else
        i2c_write_register(i2c_addresses[index] << 1, i, driver_buffers[index].pwm_buffer + i, 18, IS31FL3746A_I2C_TIMEOUT);
#    endif
#endif
    }
}
...
#ifdef IS31FL3746A_DOUBLE_BUFFER
        if (memcmp(driver_buffers[index].pwm_buffer, driver_buffers[index].pwm_flush_buffer, IS31FL3746A_PWM_REGISTER_COUNT) == 0) {
            // if they are the same return early
            return;
        }

        // copy the current buffer to the flush buffer
        memcpy(driver_buffers[index].pwm_flush_buffer, driver_buffers[index].pwm_buffer, IS31FL3746A_PWM_REGISTER_COUNT);
#endif
        is31fl3746a_select_page(index, IS31FL3746A_COMMAND_PWM);
