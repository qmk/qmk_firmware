...
void is31fl3742a_write_pwm_buffer(uint8_t addr, uint8_t index) {
    ...
    for (uint8_t i = 0; i < IS31FL3742A_PWM_REGISTER_COUNT; i += 30) {
        ...
        for (uint8_t j = 0; j < IS31FL3742A_I2C_PERSISTENCE; j++) {
            if (i2c_writeReg(addr << 1, i + 1, g_pwm_buffer[index] + i, 30, IS31FL3742A_I2C_TIMEOUT) == I2C_STATUS_SUCCESS) break;
        }
#else
        i2c_writeReg(addr << 1, i + 1, g_pwm_buffer[index] + i, 30, IS31FL3742A_I2C_TIMEOUT);
#endif
    }
}
