...
void adns5050_init(const void* config) {
    ...
    gpio_set_pin_output(adns5050_config->cs);
...
uint8_t adns5050_serial_read(adns5050_config_t* adns5050_config) {
    ...
    for (uint8_t i = 0; i < 8; ++i) {
        gpio_write_pin_low(adns5050_config->sclk);
        wait_us(1);

        byte = (byte << 1) | readPin(adns5050_config->sdio);

        gpio_write_pin_high(adns5050_config->sclk);
        wait_us(1);
    }
...
void adns5050_serial_write(adns5050_config_t* adns5050_config, uint8_t data) {
    ...
    for (int8_t b = 7; b >= 0; b--) {
        ...
        if (data & (1 << b))
            gpio_write_pin_high(adns5050_config->sdio);
        else
            gpio_write_pin_low(adns5050_config->sdio);
