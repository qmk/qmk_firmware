...
report_mouse_t azoteq_iqs5xx_get_report(report_mouse_t mouse_report) {
    static uint8_t gesture_buttons = 0;

    // Release only buttons generated by the previous IQS5xx gesture.
    mouse_report.buttons &= ~gesture_buttons;
    gesture_buttons = 0;
    ...
    if (status == I2C_STATUS_SUCCESS) {
        ...
        if (base_data.gesture_events_0.single_tap || base_data.gesture_events_0.press_and_hold) {
            ...
            gesture_buttons = pointing_device_handle_buttons(gesture_buttons, true, POINTING_DEVICE_BUTTON1);
        } else if (base_data.gesture_events_1.two_finger_tap) {
            pd_dprintf("IQS5XX - Two finger tap.\n");
            gesture_buttons = pointing_device_handle_buttons(gesture_buttons, true, POINTING_DEVICE_BUTTON2);
        } else if (base_data.gesture_events_0.swipe_x_neg) {
            pd_dprintf("IQS5XX - X-.\n");
            gesture_buttons = pointing_device_handle_buttons(gesture_buttons, true, POINTING_DEVICE_BUTTON4);
            ignore_movement = true;
        } else if (base_data.gesture_events_0.swipe_x_pos) {
            pd_dprintf("IQS5XX - X+.\n");
            gesture_buttons = pointing_device_handle_buttons(gesture_buttons, true, POINTING_DEVICE_BUTTON5);
            ignore_movement = true;
        } else if (base_data.gesture_events_0.swipe_y_neg) {
            pd_dprintf("IQS5XX - Y-.\n");
            gesture_buttons = pointing_device_handle_buttons(gesture_buttons, true, POINTING_DEVICE_BUTTON6);
            ignore_movement = true;
        } else if (base_data.gesture_events_0.swipe_y_pos) {
            pd_dprintf("IQS5XX - Y+.\n");
            gesture_buttons = pointing_device_handle_buttons(gesture_buttons, true, POINTING_DEVICE_BUTTON3);
            ignore_movement = true;
            ...
            if (AZOTEQ_IQS5XX_COMBINE_H_L_BYTES(base_data.x.h, base_data.x.l) < 0) {
                ...
                gesture_buttons = pointing_device_handle_buttons(gesture_buttons, true, POINTING_DEVICE_BUTTON7);
            } else if (AZOTEQ_IQS5XX_COMBINE_H_L_BYTES(base_data.x.h, base_data.x.l) > 0) {
                pd_dprintf("IQS5XX - Zoom in.\n");
                gesture_buttons = pointing_device_handle_buttons(gesture_buttons, true, POINTING_DEVICE_BUTTON8);
            }
        } else if (base_data.gesture_events_1.scroll) {
            pd_dprintf("IQS5XX - Scroll.\n");
            mouse_report.h = CONSTRAIN_HID(AZOTEQ_IQS5XX_COMBINE_H_L_BYTES(base_data.x.h, base_data.x.l));
            mouse_report.v = CONSTRAIN_HID(AZOTEQ_IQS5XX_COMBINE_H_L_BYTES(base_data.y.h, base_data.y.l));
        }
        if (base_data.number_of_fingers == 1 && !ignore_movement) {
            mouse_report.x = CONSTRAIN_HID_XY(AZOTEQ_IQS5XX_COMBINE_H_L_BYTES(base_data.x.h, base_data.x.l));
            mouse_report.y = CONSTRAIN_HID_XY(AZOTEQ_IQS5XX_COMBINE_H_L_BYTES(base_data.y.h, base_data.y.l));
        }
