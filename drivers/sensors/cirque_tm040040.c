...
void RAP_ReadBytes(uint8_t address, uint8_t* data, uint8_t count) {
    ...
    if (touchpad_init) {
        i2c_writeReg(CIRQUE_TM040040_ADDR << 1, cmdByte, NULL, 0, CIRQUE_TM040040_TIMEOUT);
        if (i2c_readReg(CIRQUE_TM040040_ADDR << 1, cmdByte, data, count, CIRQUE_TM040040_TIMEOUT) != I2C_STATUS_SUCCESS) {
...
void RAP_Write(uint8_t address, uint8_t data) {
    ...
    if (touchpad_init) {
        if (i2c_writeReg(CIRQUE_TM040040_ADDR << 1, cmdByte, &data, sizeof(data), CIRQUE_TM040040_TIMEOUT) != I2C_STATUS_SUCCESS) {
            dprintf("error right touchpad\n");
            touchpad_init = false;
        }
        i2c_stop();
...
void cirque_tm040040_clear_flags() {
...
void ERA_ReadBytes(uint16_t address, uint8_t* data, uint16_t count) {
    ...
    cirque_tm040040_enable_feed(false);  // Disable feed
...
void cirque_tm040040_set_adc_attenuation(uint8_t adcGain) {
    uint8_t temp = 0x00;
...
void cirque_tm040040_tune_edge_sensitivity(void) {
    uint8_t temp = 0x00;
...
void cirque_tm040040_init(void) {
    ...
    cirque_tm040040_set_adc_attenuation(0xFF);
    cirque_tm040040_tune_edge_sensitivity();
    cirque_tm040040_enable_feed(true);
}
