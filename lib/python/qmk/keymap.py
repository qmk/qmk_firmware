...
def generate_with_encoders(keyboard, layout, layers, encoders):
    ...
    Args:
        ...
        encoders
          A json object describing the encoder positions and counter- and clockwise turn values.
    """

    # First, replace the keymap
    layer_txt = []
    ...
    keymap = '\n'.join(layer_txt)
    keymap_c = DEFAULT_KEYMAP_ENCODERS_C # literally only two keyboards have templates so...

    keymap_c = keymap_c.replace('__KEYMAP_GOES_HERE__', keymap)

    # Second, replace the encoders
    encoders_txt = []
...
def write_with_encoders(keyboard, keymap, layout, layers, encoders):
    """Generate the `keymap.c` and write it to disk.
    ...
    Args:
        ...
        encoders
          A json object describing the encoder positions and their counter- and clockwise turn values.
    """
    keymap_c = generate_with_encoders(keyboard, layout, layers, encoders)
    keymap_path = qmk.path.keymap(keyboard)
    keymap_dir = os.path.join(keymap_path, keymap)
    keymap_file = os.path.join(keymap_dir, 'keymap.c')
