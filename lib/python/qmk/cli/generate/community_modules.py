...
MODULE_API_LIST = [
    ...
    ModuleAPI(ret_type='bool', name='pre_process_record', args='uint16_t keycode, keyrecord_t *record', call_params='keycode, record', ret_val='true'),
...
def generate_community_modules_h(cli):
    ...
    lines = [
        ...
        f'#define ASSERT_COMMUNITY_MODULES_API_VERSION(x) _Static_assert((x) <= COMMUNITY_MODULES_API_VERSION, "Modules API mismatch, module requires higher version of QMK with API >= {MODULE_API_VERSION}.")',
        '',
    ...
    if keymap_json and 'modules' in keymap_json:
        ...
        lines.append(f'// Parent implementation')
        for api in MODULE_API_LIST:
...
def generate_community_modules_c(cli):
    ...
    lines = [GPL2_HEADER_C_LIKE, GENERATED_HEADER_C_LIKE, '', '#include "community_modules.h"']
    keymap_json = parse_configurator_json(cli.args.filename)
    if keymap_json and 'modules' in keymap_json:
        for module in keymap_json['modules']:
            lines.append('')
