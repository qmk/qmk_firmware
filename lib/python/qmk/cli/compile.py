...
def compile(cli):
    ...
    if cli.args.filename:
        ...
        command = compile_configurator_json(user_keymap, parallel=cli.config.compile.parallel if cli.config.flash.parallel else 1, **envs)

    else:
        if cli.config.compile.keyboard and cli.config.compile.keymap:
            # Generate the make command for a specific keyboard/keymap.
            command = create_make_command(cli.config.compile.keyboard, cli.config.compile.keymap, parallel=cli.config.compile.parallel if cli.config.flash.parallel else 1, **envs)
