...
can_import_deps = True
try:
    from io import BytesIO
    from qmk.painter_qff import QFFFont
    from qmk.painter import render_header, render_source, render_license, render_bytes
except ImportError:
    can_import_deps = False

# Only import Quantum Painter related things if PIL is present.
if can_import_deps:  # noqa: Conditional based on module presence
    ...
    @cli.subcommand('Converts an input font to something QMK understands')
    def painter_make_font_image(cli):
        from qmk.painter import missing_deps_warning
        missing_deps_warning(cli)
        return 1
