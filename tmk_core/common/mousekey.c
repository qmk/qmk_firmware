...
void mousekey_task(void) {
    ...
    if (timer_elapsed(last_wheel_timer) >= (mousekey_wheel_repeat ? mk_wheel_interval : mk_wheel_delay * 10)) {
        if (mouse_wheel_report.v || mouse_wheel_report.h) {
            ...
            if (mouse_wheel_report.v > 0)
                mouse_wheel_report.v = wheel_unit();
            if (mouse_wheel_report.v < 0)
                mouse_wheel_report.v = wheel_unit() * -1;
            if (mouse_wheel_report.h > 0)
                mouse_wheel_report.h = wheel_unit();
            if (mouse_wheel_report.h < 0)
                mouse_wheel_report.h = wheel_unit() * -1;
            mousekey_send_wheel();
        }
    }
}
...
#    ifndef MOUSEKEY_PERSISTENT_ACCEL
        ...
        mousekey_set_accel(mousekey_accel | (1 << 0));
    else if (code == KC_MS_ACCEL1)
        mousekey_set_accel(mousekey_accel | (1 << 1));
    else if (code == KC_MS_ACCEL2)
        mousekey_set_accel(mousekey_accel | (1 << 2));
#    else
    else if (code == KC_MS_ACCEL0)
        mousekey_set_accel(1 << 0);
    else if (code == KC_MS_ACCEL1)
        mousekey_set_accel(1 << 1);
    else if (code == KC_MS_ACCEL2)
        mousekey_set_accel(1 << 2);
#    endif
}
...
void mousekey_off(uint8_t code) {
    if (code == KC_MS_UP && mouse_report.y < 0)
        ...
        mouse_report.x = 0;
    else if (code == KC_MS_WH_UP && mouse_wheel_report.v > 0)
        mouse_wheel_report.v = 0;
    else if (code == KC_MS_WH_DOWN && mouse_wheel_report.v < 0)
        mouse_wheel_report.v = 0;
    else if (code == KC_MS_WH_LEFT && mouse_wheel_report.h < 0)
        mouse_wheel_report.h = 0;
    else if (code == KC_MS_WH_RIGHT && mouse_wheel_report.h > 0)
        mouse_wheel_report.h = 0;
    else if (code == KC_MS_BTN1)
        mouse_report.buttons &= ~MOUSE_BTN1;
    else if (code == KC_MS_BTN2)
...
static void mousekey_debug(void) {
    ...
    print(" ");
    print_decs(mouse_wheel_report.v);
    print(" ");
    print_decs(mouse_wheel_report.h);
    print("](");
