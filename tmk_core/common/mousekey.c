...
#endif
static uint8_t mousekey_accel = mkspd_med;
static uint16_t last_timer   = 0;
...
void mousekey_task(void) {
  ...
  if ((mouse_report.x  ||  mouse_report.y)  &&  timer_elapsed(last_timer_c) > c_intervals[mousekey_accel]) {
  mouse_report.h = 0;
  mouse_report.v = 0;
  mousekey_send();
  ...
  if ((mouse_report.h  ||  mouse_report.v)  &&  timer_elapsed(last_timer_w) > w_intervals[mousekey_accel]) {
  mouse_report.x = 0;
  mouse_report.y = 0;
  mousekey_send();
...
void adjust_speed(void) {
  uint16_t const c_offset = c_offsets[mousekey_accel];
  uint16_t const w_offset = w_offsets[mousekey_accel];
...
void mousekey_on(uint8_t code) {
  ...
  uint16_t const c_offset = c_offsets[mousekey_accel];
  uint16_t const w_offset = w_offsets[mousekey_accel];
  uint8_t const old_speed = mousekey_accel;
#endif
...
#ifdef MK_3_SPEED
  else if (code == KC_MS_ACCEL0)   mousekey_accel = mkspd_slow;
  else if (code == KC_MS_ACCEL1)   mousekey_accel = mkspd_med;
  else if (code == KC_MS_ACCEL2)   mousekey_accel = mkspd_fast;
  if (mousekey_accel != old_speed) adjust_speed();
#else
...
#else /* #ifndef MK_3_SPEED */
#ifdef MK_MOMENTARY_ACCEL
  else if (code == KC_MS_ACCEL0) mousekey_accel = mkspd_DEFAULT;
  else if (code == KC_MS_ACCEL1) mousekey_accel = mkspd_DEFAULT;
  else if (code == KC_MS_ACCEL2) mousekey_accel = mkspd_DEFAULT;
  if (mousekey_accel != old_speed) adjust_speed();
#endif /* #ifdef MK_MOMENTARY_ACCEL */
#endif /* #ifndef MK_3_SPEED */
}
