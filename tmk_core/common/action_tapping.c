...
__attribute__((weak)) uint16_t get_tapping_term(uint16_t keycode, const keyrecord_t *record) { return TAPPING_TERM; }
...
#    ifdef COMBO_ENABLE
        if (timer_expired(tapping_key.event.time, keyp->event.time)) {
            debug("Tapping: Key delayed due to combo term: ");
            debug_event(event);
            debug("\n");
            process_record(keyp);
            return true;
