...
#define KEYBOARD_EXT_ATTR_INIT_(id_init, locale_index, ...) {   \
    id_init                                           \
    .form_factor = KEYBOARD_FORM_FACTOR,              \
    .key_type = KEYBOARD_KEY_TYPE,                    \
    .physical_layout = KEYBOARD_PHYSICAL_LAYOUT,      \
    .vendor_physical_layout = KEYBOARD_VENDOR_LAYOUT, \
    .ietf_language_tag_index = locale_index,          \
    .implemented_assist_controls = KEYBOARD_IMPLEMENTED_INPUT_ASSIST_CONTROLS, \
    __VA_ARGS__                                       \
}
/**
 ...
 */
#define KEYBOARD_EXT_ATTR_INIT(locale_index, ...) KEYBOARD_EXT_ATTR_INIT_( \
        KEYBOARD_EXT_ATTR_REPORT_ID_INIT(REPORT_ID_KEYBOARD),  \
        KEYBOARD_EXT_ATTR_LOCALE(locale_index), \
        __VA_ARGS__) // override defaults
