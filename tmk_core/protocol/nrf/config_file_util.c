    ...
    const char * json)
{
  ...
  else if (inst->tail[0] != '\0')
  {
    ...
    if (inst->keymap_idx >= inst->keymap_len) { return 1; }
...
int32_t get_val_u8array(const char* json, const char* key,
    uint8_t* array, uint32_t* const len)
{
  char* str;
  char* str2;
    ...
    char* str, uint32_t len)
{
  ...
  snprintf(str, len, "{\"config\":{\"version\":%lu,\r\n\"device_info\":"
  ...
  "\"reserved\":[%s]}}",
      config->version,
      (uint32_t)config->device_info.vid,
      (uint32_t)config->device_info.pid,
      config->device_info.name,
      config->device_info.manufacture,
      config->device_info.description,
      config->matrix.rows,
      config->matrix.cols,
      config->matrix.device_rows,
      config->matrix.device_cols,
      ...
      config->param_central.max_interval,
      config->param_central.min_interval,
      config->param_central.slave_latency,
      config->led.pin,
      config->led.num,
      str_locale,
