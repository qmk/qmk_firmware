...
const uint16_t digit_codes[] = {KC_1, KC_2, KC_3, KC_4, KC_5, KC_6, KC_7, KC_8, KC_9, KC_0};
const uint16_t letter_codes[] = {KC_A, KC_B, KC_C, KC_D, KC_E, KC_F, KC_G, KC_H, KC_I, KC_J, KC_K,
                                 KC_L, KC_M, KC_N, KC_O, KC_P, KC_Q, KC_R, KC_S, KC_T, KC_U, KC_V,
                                 KC_W, KC_X, KC_Y, KC_Z};
static uint16_t random_digit;
static uint16_t random_letter;
...
const macro_t *action_get_macro(keyrecord_t *record, uint8_t id, uint8_t opt)
{
  ...
  case M_USERNAME:
    ...
    break;
  case M_RANDDIGIT:
    /* Generate a keystroke for a digit chosen at random */
    if (record->event.pressed) {
      random_digit = digit_codes[rand()%(sizeof(digit_codes) / sizeof(uint16_t))];
      register_code(random_digit);
    } else {
      unregister_code(random_digit);
    }
    break;
  case M_RANDLETTER:
    /* Generate a keystroke for a letter chosen at random */
    if (record->event.pressed) {
      random_letter = letter_codes[rand()%(sizeof(letter_codes) / sizeof(uint16_t))];
      register_code(random_letter);
    } else {
      unregister_code(random_letter);
    }
    break;
  }
  return MACRO_NONE;
