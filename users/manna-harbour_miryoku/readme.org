...
For each half: the top row, outer column, inner encoder, and 2 outward most keys on bottom row are unused.

Keyboards supporting this layout: sofle, keycapsss/kimiko, tkw/grandiceps.

Example build command lines:

#+BEGIN_SRC sh :tangle no
make sofle:manna-harbour_miryoku:flash # sofle
make keycapsss/kimiko:manna-harbour_miryoku:flash # keycapsss/kimiko
#+END_SRC
