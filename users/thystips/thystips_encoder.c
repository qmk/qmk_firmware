...
void encoder_action_navtab(bool clockwise) {
    if (clockwise)
...
bool encoder_update_user(uint8_t index, bool clockwise) {
    ...
    } else if (mods_state & MOD_BIT(KC_RALT)) { // If holding Right Alt, navigate next/prev tab
        unregister_mods(MOD_BIT(KC_RALT));
        encoder_action_navtab(clockwise);
