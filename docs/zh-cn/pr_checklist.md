...
本文将略述 QMK 协作者（Collaborators）在审阅一个PR时会查验的主要内容。
...
- 新目录及文件的名称须为小写
    - 对于其原始名称就含有大写的情况（比如 LUFA，ChibiOS，或从其它仓库引用来的文件等），本规则可不适用
    - 若确有正当的理由（即为了与核心文件保持一致等），本规则可不适用
        - 键盘的设计者命名时使用了大写字母，不是一个足够正当的理由
- 所有的 `*.c` 和 `*.h` 源文件头部须有开源协议头
    - 基于一致性考虑，推荐使用 GPL2/GPL3
    - 可以从本文最下方拷贝 GPL2+ 开源协议头的范本
    - 允许使用其它开源协议，但必须与 GPL 兼容且允许二次分发。但使用特异的开源协议几乎肯定会导致 PR 的合入被推迟
    - 文件头部缺失开源协议时会因为无法确认开源协议是否会有冲突，从而导致 PR 的合入受阻
- 遵循 QMK 仓库的“最佳实践”
    - 这只是一份概略性的清单，并且会随着时间的推移进行修订
