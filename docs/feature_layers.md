...
* `DF(layer)` - switches the default layer. The default layer is the always-active base layer that other layers stack on top of. See below for more about the default layer. For instance, this could be used to switch from QWERTY to Dvorak layout. This persists after power loss, but can be changed by resetting the EEPROM.
