...
Обратите внимание, что слои, как правило, не являются независимыми друг от друга абстракциями, и несколько слоёв могут быть активны одновременно. 
Для того чтобы это работало, используется "прозрачность", обозначающаяся как "KC_TRNS", чтобы прошивке "брала" символы с более "низких" уровней. 
При использовании мгновенного переключения уровней с помощью MO(), LM(), TT() или LT() оставляйте кей-код на указанных выше слоях "прозрачным", иначе он будет работать не так, как задумано.
...
Это может быть использовано для переключения с раскладки QWERTY на раскладку Дворака. (Обратите внимание, что это действует только до тех пор, пока клавиатура не отключится. 
Для постоянного изменения уровня по умолчанию требуется более глубокая настройка, например, вызов функции set_single_persistent_default_layer внутри process_record_user.)

**МО**(№ слоя) - мгновенно активирует указанный в скобках слой. Как только вы отпускаете клавишу, слой деактивируется.

**LM(№ слоя, мод) - Мгновенно активирует слой (как и "МО"), но со включёнными на них модификаторами. 
...
**LT**(№слоя, kc) - мгновенно активирует слой при удерживании и отправляет кей-код при нажатии. Поддерживает только уровни 0-15. //зачем активировать какой-то слой, если отправляется конкретный, указанный кей-код ?//

**OSL**(уровень) - на мгновение активирует слой до нажатия следующей клавиши. (См. подробные сведения и дополнительные функции в разделе «One Shot Keys»).

**TG**(№слоя) - переключает на слой, активируя его, если он неактивен, и наоборот //т.е выключает его, если активен ??//
...
В настоящее время LT() и MT() ограничены набором "базовых"/?/ клави-кодов, что означает, что вы не можете использовать такие кей-коды, как LCTL(), KC_TILD или что-либо большее, чем 0xFF. 
...
Layer Tap использует 4 бита для слоя (и именно поэтому он ограничен слоями 0-15, на самом деле), в то время как Mod Tap делает то же самое, 4 бита для идентификатора, 
4 бита, для модификатора, и 8 бит для кей-кода. Из-за этого используемый кей-код ограничен 0xFF (0-255) битами, т.е. "основными"//так?// кей-кодами.
 
Переход к 32-битному коду для каждой из клавиш решит многие из этих проблем, но удвоит объем пространства, используемого матрицей раскладки клавиатуры. И это тоже может вызвать проблемы. 
Если вам нужно применить модификаторы к нажатой клавише, для этого можно использовать Tap Dance.

Кроме того, если в Mod Tap или Layer Tap указан хотя бы один правосторонний модификатор, это приведет к тому, что все указанные модификаторы станут правосторонними, поэтому их невозможно смешать и сопоставить//?//.
...
Если вы только начинаете работать с QMK, вам захочется, чтобы все было просто. Следуйте этим рекомендациям при настройке слоев:
     ...
     В раскладке слоя ссылаться только на слои с более высокими номерами. Поскольку уровни обрабатываются начиная с активного уровня с самым высоким номером (самого верхнего), 
     изменение состояния нижних уровней может быть сложным и подверженным ошибкам.
...
Когда вы хорошо почувствуете, как работают слои и что вы можете делать, вы сможете проявить больше творчества. 
Правила, перечисленные в разделе для начинающих, помогут избежать некоторых непростых моментов, но они могут вас заметно ограничивать, особенно в случаях сверхкомпактных клавиатур. 
...
В результате, если вы активируете слой, численно меньший, чем ваш текущий слой, и ваш текущий слой (или другой слой, который активен и выше вашего целевого слоя) имеет что-то другое, кроме KC_TRNS, это ключ, который будет отправлен, не ключ на только что активированном слое. 
Это проблема у большинства, задающих вопрос «почему не переключается мой уровень?».

Иногда вам может потребоваться переключаться между слоями в макросе или как часть функции tap dance. 
Layer_on активирует слой, а layer_off деактивирует его. Больше функций, связанных со слоями, можно найти в action_layer.h.
...
Это передает состояние слоя функции, где его можно прочитать или изменить.
...
layer_state_cmp (state, layer)	Проверяет состояние, чтобы увидеть, включен ли указанный слой.  IS_LAYER_ON_STATE (состояние, уровень), IS_LAYER_OFF_STATE (состояние, уровень)
				                        Предназначен для использования в обратных вызовах слоев.

В дополнение к функциям, которые вы можете вызывать, существует ряд функций обратного вызова, которые вызываются каждый раз при изменении слоя. Это передает состояние слоя в функцию, где оно может быть прочитано и изменено.
