...
Для отражения нажатой клавиши на уровне сигнала, обычно, все старшие 8 бит в action code(коде команды) есть 0, а "младшие" 8 бит содержат код использования USB HID, а точнее кей-код сгенерированный для клавиши. // не скан-код?
...
Определение текущего слоя раскладки зависит от двух 32-битных параметров:
     ...
     default_layer_state - указывает базовый слой раскладки клавиатуры (0-31), который всегда(?) действителен и на который следует ссылаться (уровень по умолчанию). ***почему базовый слой не равен 0?
