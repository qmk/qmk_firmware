...
For 4× encoders, you also can assign default position if an encoder skips pulses when it changes direction. For example, if your encoder send high level on both pins by default, define this:
...
You could even support three encoders using only three pins (one per encoder). However, in this configuration, rotating two encoders which share pins simultaneously will often generate incorrect outputs. For example:
```
