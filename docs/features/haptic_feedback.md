...
* `LT()` layer tap keys, when held to activate a layer. However, when tapped and quickly released and the key sends a keycode, haptic feedback is still triggered.
* `TT()` layer tap toggle keys, when held to activate a layer. However, when tapped `TAPPING_TOGGLE` many times to permanently toggle the layer, haptic feedback is still triggered on the last tap.
* `MT()` Mod-Tap keys, when held to keep a usual modifier key pressed. However, when tapped and quickly released and the key sends a keycode, haptic feedback is still triggered. See also [Mod-Tap](../mod_tap).
