...
Pro Micro (Atmega32u4), make sure to include `CONFIG_USB_ACM=y`. Other devices may require `USB_SERIAL` and any of its suboptions.

## Unknown Device for DFU Bootloader

Issues encountered when flashing keyboards on Windows are most often due to having the wrong (or no) drivers installed for the bootloader.
...
## USB VID and PID
You can use any ID you want with editing `config.h`. Using any presumably unused ID will be no problem in fact, except for the very low chance of collision with other products.
...
If bootmagic is enabled in any form, you should be able to do this (see [Bootmagic docs](features/bootmagic) and keyboard info for specifics on how to do this).
