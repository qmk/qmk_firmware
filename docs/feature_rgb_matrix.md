...
?> `RGB_*` keycodes cannot be used with functions like `tap_code16(RGB_HUD)` inside `encoder_update_user()` or `process_record_user()`. RGB [functions](#functions-idfunctions) should be implemented in their place.
