...
<!---
  original document: 0.14.14:docs/feature_auto_shift.md
  git diff 0.14.14 HEAD -- docs/feature_auto_shift.md | cat
-->
...
キーをタップする時に、キーを放す前にほんの短い間押したままにします。この押したままにする時間は全ての人にとって異なる長さです。自動シフトは、定数 `AUTO_SHIFT_TIMEOUT` を定義し、これは普段の押された状態の時間の2倍に通常は設定されます。タイマーは、キーを押す時に開始され、`AUTO_SHIFT_TIMEOUT` の時間が過ぎてもキーを放さなかった場合、キーのシフトバージョンが発行されます。時間が `AUTO_SHIFT_TIMEOUT` 時間よりも短い場合は、通常の状態が発行されます。

`AUTO_SHIFT_REPEAT`が定義された場合、キーリピートがサポートされます。キーを押し続けるとシフトされたキーが繰り返されますが、これは `AUTO_SHIFT_NO_AUTO_REPEAT` で無効にできます。通常のキーを繰り返したい場合は、キーを一度タップし、すぐに( `TAPPING_TERM` 内で)もう一度押し続けます(オートリピートが無効の場合は、シフトされた値でも動作します)。

## 自動シフトには制限がありますか？

残念ながらあります。

シフトをするつもりがない時にシフトされた文字を取得し、シフトしたい時にそうではない他の文字を取得するでしょう。これは結局は練習になります。急いでいる時は、シフトされたバージョンのために十分長くキーを押したと思うかもしれませんが、そうではありませんでした。一方、キーをタップしていると思うかもしれませんが、実際には予想よりも少し長い間押していました。

さらに、キーリピートを使うと、目的のシフト状態が混ざってしまうことがあります。シフト状態は、常に最後に押されたキーに「属します」。例えば、大文字をキーリピートしてから小文字をタップすると(それが自動シフトキーかどうかに関係なく)大文字の*キー*は保持されたままになりますが、シフトはされません。
...
これは、タップホールドキーと同じように、どのキーを自動シフトさせるかを設定できる関数です。
...
```
この機能はデフォルトで有効になっており、定義する必要はありません。

### AUTO_SHIFT_REPEAT (簡単な定義)

キーリピートを有効にします。

### AUTO_SHIFT_NO_AUTO_REPEAT (簡単な定義)

`AUTO_SHIFT_TIMEOUT` を超えた場合、自動的にキーリピートを無効にします。
