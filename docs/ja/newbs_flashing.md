...
カスタムファームウェアは出来たので、キーボードに書き込み(フラッシュし)ましょう。
...
Finder または Explorer でファームウェアのファイルを探します。
ファイル名は `.hex` または `.bin` のどちらかの形式です。
ビルド時に QMK は、キーボードに適した形式のものを qmk_firmware のトップフォルダにコピーしているはずです。

Windows か macOS を使用しているときは現在のファームウェアフォルダを Explorer か Finder で簡単に開くためのコマンドがあります。
...
ファームウェアの書き込み中にキーボードのプラグを抜いたり、書き込みプロセスを中断したりしないことが非常に重要です。
...
使用しているブートローダーがわかっている場合は、ファームウェアをコンパイルするときに、実際に `make` コマンドに追加のテキストを追加して、書き込みプロセスを自動化できます。
