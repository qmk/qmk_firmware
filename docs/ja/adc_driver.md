...
QMK は、サポートされる MUC のアナログ-to-デジタルコンバータ (ADC) を使って、特定のピンの電圧を計測することができます。これは、[ロータリーエンコーダ](ja/feature_encoders.md)とは対照的に、Bluetooth キーボードのバッテリーレベルインジケータや、可変抵抗器を使ったボリューム制御などの実装に役立ちます。

このドライバは現在 AVR と一部の AVR デバイスをサポートしています。返される値は 0V と VCC (通常 AVRの場合は 5V または 3.3V、ARM の場合は 3.3V)の間でマッピングされた 10ビットの整数 (0-1023) ですが、ARM の場合、もしより精度が必要であれば `#define` を使うと操作の制御がより柔軟性になります。
