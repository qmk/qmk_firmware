...
現在のモディファイアの状態は、2つの関数によって主にアクセスされます。: `get_mods()` 関数は通常のモディファイアの状態を、`get_oneshot_mods()` 関数はワンショットモディファイアの状態を確認する関数です。(キーが押されていない限り、それらのキーは通常のモディファイアキーのように行動します。)

現在のモディファイアの状態に含まれる1つまたはそれ以上の特定のモディファイアの存在は、あなたがマッチさせたいモディファイアの組み合わせに相当するモッドマスクと、モディファイアの状態の論理積によって検知されます。
ビット演算が使われる理由は、モディファイアの状態が (GASC)<sub>R</sub>(GASC)<sub>L</sub> フォーマットにおいてシングルバイトとして保存されるためです。
...
左Alt を `KC_TAB` のように振る舞う `KC_ESC` と組み合わせて、Alt-Tab でのアプリ切り替えを実現する単純な例です。この例は、左Alt だけがアクティブになっているか厳格に確認します。つまり、Alt+Shift+Esc によるアプリの逆順での切り替えはできないという意味です。また、この例は、AltGr+Escape キーボードショートカットを行う能力は維持しますが、現実の Alt+Escape キーボードショートカットを起動する能力を取り除くことに留意してください。
