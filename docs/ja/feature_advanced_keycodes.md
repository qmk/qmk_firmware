...
| `MOD_HYPR` | Hyper (左 Control、左 Shift、左 Alt および左 GUI) |
| `MOD_MEH` | Meh (左 Control、左 Shift および 左 Alt) |
...
* `OSM(mod)` - *mod*を一時的に押し続けます。[モッドタップ](#mod-tap)で示したように、`KC_*` コードでは無く、`MOD_*` キーコードを使わなければなりません。
...
ワンショットモッドについては、設定するために `set_oneshot_mods(MOD)` を呼び出し、キャンセルするために `clear_oneshot_mods()` を呼び出す必要があります。
...
モッドタップキーを押し、他のキーをタップ(押して放す)して、モッドタップキーを放すと、全てのタッピング期間内で両方のキーの "タッピング"機能が出力されます。
...
?> `モッドタップ割り込みの無視`を有効にしている場合、これにより両方の動きが変更されます。通常のキーには、最初のキーが最初に放された場合、あるいは両方のキーが `TAPPING_TERM` より長くホールドされた場合に、モディファイアが追加されます。
...
通常、これは `X` を送信します (`SHIFT`+`x`). `モッドタップ割り込みの無視`を有効にすると、ホールド アクションを登録するために、`TAPPING_TERM` に両方のキーをホールドする必要があります。この場合、高速なタップは `ax` を送信しますが、両方をホールドすると、`X`  (`SHIFT`+`x`) を出力します。
