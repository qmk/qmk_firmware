...
さらに、デフォルトでは、`pointing_device_send()` はレポートが実際に変更された場合のみレポートを送信します。これにより、マウスレポートが継続的に送信されなくなり、ホストシステムが起動されたままになります。この動作は、独自の `pointing_device_send()` 関数を作成することで変更できます。

また、`has_mouse_report_changed(new, old)` 関数を使って、レポートが変更されたかどうかを確認します。
