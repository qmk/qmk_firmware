...
この機能により、外部ドライバによって駆動される LED マトリクッスを使うことができます。バックライトシステムにフックするため、これを制御するためにバックライトと同じキーコードを使うことができます。
...
I2C IS31FL3731 RGB コントローラを使ったアドレス指定可能な LED マトリックライトのための基本的なサポートがあります:有効にするには、`rules.mk` に以下を追加します:
