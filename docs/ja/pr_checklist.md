...
- QMKコードベースの「ベストプラクティス」に従う
    ...
    - 旧式の GPIO/I2C/SPI 関数を使用しない - 正当な理由がない限り、QMKの抽象化を使用しなければなりません (怠惰は正当な理由にはなりません)
...
- `keyboard.c`
    ...
    - `matrix_init_board()` などが `keyboard_pre_init_kb()` に移行されました。[keyboard_pre_init*](https://docs.qmk.fm/#/custom_quantum_functions?id=keyboard_pre_init_-function-documentation) を参照してください
