...
`TAPPING_TERM` の時間は、タップダンスキーのタップとタップの間の最大許容時間で、ミリ秒単位で計測されます。例えば、もし、あなたがこの上にある `#define` ステートメントを使い、1回タップすると `Space` が送信され、2回タップすると `Enter` が送信されるタップダンスキーをセットアップした場合、175ミリ秒以内に2回キーをタップすれば `ENT` だけが送信されるでしょう。もし、1回タップしてから175ミリ秒以上待ってからもう一度タップすると、`SPC SPC` が送信されます。
`TAPPING_TERM_PER_KEY` は、[カスタムの `get_tapping_term` 関数](ja/tap_hold.md#tapping_term)を介してタッピング期間を制御する場合にのみ必要です。これは、`TAPPING_TERM` がタップダンスキーだけでなく、他のキーにも影響を与えるためです。
...
* `ACTION_TAP_DANCE_LAYER_TOGGLE(kc, layer)`: 1回タップすると `kc` キーコードが送信され、2回タップすると `layer` の状態を切り替えます(これは `TG` レイヤーキーコードのように機能します)。
* `ACTION_TAP_DANCE_FN(fn)`: 最終的なタップダンスの実行回数を使用して、ユーザーキーマップで定義した指定の関数を呼び出します。
...
* `ACTION_TAP_DANCE_FN_ADVANCED_WITH_RELEASE(on_each_tap_fn, on_each_release_fn, on_dance_finished_fn, on_dance_reset_fn)`: このマクロは、`on_each_release_fn` を追加した `ACTION_TAP_DANCE_FN_ADVANCED` と同じです。`on_each_release_fn` は、タップダンスのキーが離されるたびに呼び出されます。注目すべき点は、タップダンスのキーが完了し離された場合でも、`on_each_release_fn` は、呼び出されます（例: ダンスが終了した後にキーが解放された場合）。
...
最初のオプションに似ていますが、2番目と3番目のオプションは単純なレイヤー切替のケースに適しています。
...
#### 例1: 100回タップした後に "Safety Dance!" を送信します :id=example-1
...
#### 例2: 1つずつ LED を点灯させてから消灯する :id=example-2
...
// 全てのタップダンス関数をまとめたものです。この例2は "CT_FLSH" です。
tap_dance_action_t tap_dance_actions[] = {
    [TD_ESC_CAPS] = ACTION_TAP_DANCE_DOUBLE(KC_ESC, KC_CAPS),
    [CT_EGG] = ACTION_TAP_DANCE_FN(dance_egg),
...
#### 例3: タップで `:` を送信し、ホールドで `;` を送信する :id=example-3

少し手間をかければ、タップホールド構成をタップダンスとして実装することができます。できるだけ早くタップを発生させるには、タップ ダンス キーが離されるに応じて動作する必要があります。タップ ダンス フレームワークにはこれに対するコールバックがないため、 `process_record_user()` を使用します。
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    switch (keycode) {
        case TD(CT_CLN):  // タップホールド構成のすべてのタップ ダンス キーコードをリストする
