...
明らかにこれは混乱する可能性があるため、QMK は多くのキーボードレイアウトに言語固有のキーコードのエイリアスを提供します。これらは、それだけではあまり多くの事を行いません - OS の設定で対応するキーボードレイアウトをまだまだ設定する必要があります。それらをキーマップのキーキャップラベルと考えてください。
...
多くのレイアウトでは、Grave または Tilde などの特定の文字を[デッドキー](https://en.wikipedia.org/wiki/Dead_key)としてのみ使えるようにするため、送信したい文字列の中でそれのすぐあとでスペースを追加して、次の文字と潜在的に結合されることを防ぎます。
