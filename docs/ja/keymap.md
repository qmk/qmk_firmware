...
各レイヤは同時に検証することができます。レイヤーには 0 から 31 までのインデックスが付けられ、上位のレイヤーが優先されます。

    キーマップ: 32 レイヤー                 レイヤー: アクションコードマトリックス
    -----------------                   ---------------------
    レイヤーのスタック                  array_of_action_code[row][column]
        ____________ 優先順位                 _______________________
       /           / | 高い                  / ESC / F1  / F2  / F3   ....
    31 /___________// |                     /-----/-----/-----/-----
...
一方、`layer_state` を変更して、ナビゲーションキー、ファンクションキー (F1-F12)、メディアキー、特別なアクションなどの機能のベースレイヤーを他のレイヤーにオーバーレイすることができます。
...
### レイヤーの優先順位と透明性
***上位のレイヤーはレイヤーのスタックでより高い優先順位を持つ***ことに注意してください。つまり、ファームウェアはキーコードを最上位から最下位まで検索します。レイヤーで **`KC_TRNS`**(透明)以外のキーコードを見つけると、検索を中止し、下位レイヤーは参照されません。

レイアウトの一部だけを変更するオーバーレイレイヤーに `KC_TRANS` を配置して、下位レイヤーまたはベースレイヤーにフォールバックすることができます。
...
`keymap.c` ファイルには、以下の3つの主要なセクションがあります:

* [定義](#definitions)
* [レイヤー/キーマップデータ構造](#layers-and-keymaps)
* もしある場合は[カスタム関数](#custom-functions)
...
ファイルの上部に以下のものがあります:
    ...
    /* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
    *  KC_TRNS (transparent) の代わりに _______ を使うことができます   *
    ...
    * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
    ...
    // レイヤー名は同じ長さである必要はありません。
    // またそれらを完全に省いて単に数字を使うことができます。    #define _BL 0
    #define _FL 1
    #define _CL 2
...
このファイルの主要部分は `keymaps[]` 定義です。ここで、レイヤーとそれらの内容をリストします。ファイルのこの部分は、以下の定義から始まります:

    const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {

この後で、LAYOUT() マクロのリストがあります。LAYOUT() は単一のレイヤーを定義するためのキーのリストです。通常、1つ以上の"基本レイヤー" (QWERTY、Dvorak、Colemak など)があり、その上に1つ以上の"機能"レイヤーを重ねます。レイヤーの処理方法のせいで、"より上位"のレイヤーの上に"より下位"のレイヤーを重ねることはできません。

QMK の `keymaps[][MATRIX_ROWS][MATRIX_COLS]` は、16ビットのアクションコード( quantum キーコードとも呼ばれる)を保持します。典型的なキーを表すキーコードの場合、その上位バイトは0で、その下位バイトはキーボードの USB HID usage ID です。

> QMK がフォークされた TMK は、代わりに `const uint8_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS]` を使い、8ビットキーコードを保持します。一部のキーコード値は、`fn_actions[]` 配列を介して特定のアクションコードの実行を引き起こすために予約されています。

#### 基本レイヤー

Clueboard のベースレイヤーの例です:
