...
There are two notable exceptions: the Caterina bootloader, usually seen on Pro Micros, and the HalfKay bootloader shipped with PJRC Teensys, which appear as a serial port and a generic HID device respectively, and so do not require a driver.
...
Finally, unplug and replug the keyboard to make sure the new driver has been loaded. If you are using the QMK Toolbox to flash, exit and restart Toolbox too, as it can sometimes fail to recognize the driver change.
