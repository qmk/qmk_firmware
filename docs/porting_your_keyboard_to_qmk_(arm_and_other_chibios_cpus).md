...
To define a matrix where each pin is tied to ground, instead of using diodes, use `DIRECT_PINS`. The mapping defines the pins of each switch in rows and columns, from left to right. Overrides the behaviour of `DIODE_DIRECTION`, `MATRIX_ROW_PINS` and `MATRIX_COL_PINS`.
