...
If you just want to get things back to normal, you can flash only a bootloader from [`util/` folder](https://github.com/qmk/qmk_firmware/tree/master/util), and use your normal process to flash the firmware. Be sure to flash the correct bootloader for your chip:
...
If you're not sure, look in the `rules.mk` file for the keyboard in QMK - the `MCU =` will have the value you need. It may differ between versions of the board.
...
2. Select the correct bootloader/production .hex file with the `Open` dialog
