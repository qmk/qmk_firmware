...
#### `key_override_make_basic(modifiers, key, replacement)`
Sends `replacement` (can be a key-modifer combination), when `key` is pressed while `modifiers` are pressed down.

#### `key_override_make_for_layers(modifiers, key, replacement, layers)`
Additionally takes a bitmask `layers` that defines on which layers the override is used.

#### `key_override_make_for_layers_and_negative_mods(modifiers, key, replacement, layers, negative_mods)`
...
```c
key_override_t delete_key_override = key_override_make_basic(MOD_MASK_SHIFT, KC_BSPACE, KC_DELETE);
...
Continuing with the example of `shift` + `backspace` = `delete`, this section shows how a key override works in detail. Understanding this is essential to be able to take full advantage of all the options offered by key overrides.

When the necessary keys are pressed (`shift` + `backspace`), the override is 'activated' and the replacement key (+ optionally extra modifiers) is sent in the keyboard report (`delete`), while the trigger key (`backspace`) is removed from the keyboard report. The trigger modifiers may also be removed from the keyboard report upon activation of an override. In this example, the `shift` key has to be removed from the keyboard report.

The override is 'deactivated' in certain customizable situations, like when one of the trigger keys (`shift` or `backspace`) is lifted. The replacement key (`delete`) is then removed from the keyboard report and whichever keys are still held down may be re-added to the keyboard report (this is customizable).
...
```c
key_override_t next_track_override = key_override_make_for_layers_and_negative_mods(
...
key_override_t prev_track_override = key_override_make_for_layers_and_negative_mods(
    MOD_MASK_CS,
    KC_MEDIA_PLAY_PAUSE,
    KC_MEDIA_PREV_TRACK,
    ~0,
...
key_override_t vol_up_override = key_override_make_for_layers_and_negative_mods(
    MOD_MASK_ALT,
    KC_MEDIA_PLAY_PAUSE,
    KC__VOLUP,
    ~0,
    MOD_MASK_CS);
key_override_t vol_down_override = key_override_make_for_layers_and_negative_mods(
    MOD_MASK_SA,
    KC_MEDIA_PLAY_PAUSE,
    KC__VOLDOWN,
    ~0,
...
key_override_t brightness_up_override = key_override_make_for_layers_and_negative_mods(
    MOD_MASK_CA,
    KC_MEDIA_PLAY_PAUSE,
    KC_BRIGHTNESS_UP,
    ~0,
    MOD_MASK_SHIFT);
key_override_t brightness_down_override = key_override_make_basic(MOD_MASK_CSA, KC_MEDIA_PLAY_PAUSE, KC_BRIGHTNESS_DOWN);
...
```c
// Shift + esc = ~
key_override_t tilde_esc_override = key_override_make_basic(
	MOD_MASK_SHIFT,
	KC_ESC,
	S(KC_GRAVE)
);
...
// GUI + esc = `
key_override_t grave_esc_override = key_override_make_basic(
	MOD_MASK_GUI,
	KC_ESC,
	KC_GRAVE
);
...
## `key_override_t` Reference

Advanced users may need more customization than what is offered by the simple `key_override_make` initializers. For this, directly create a `key_override_t` value and set all members. Below is a reference for all members of `key_override_t`.

| Member                                                 | Description                                                                                                                                                                                                                                                                                                                                                                                                                                        |
|--------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `uint16_t trigger`                                     | The basic keycode that triggers the override. This keycode MUST exclude modifiers. It can also be a custom keycode.                                                                                                                                                                                                                                                                                                                                |
...
| `bool one_mod`                                         | If set to true, any of the modifiers in trigger_modifiers will be enough to activate the override (logical OR of modifiers). If set to false all the modifiers in trigger_modifiers have to be pressed (logical AND of modifiers).                                                                                                                                                                                                                 |
...
| `bool allows_other_keys_down_at_trigger`               | Whether allowing to activate this override when other keys are already down. This only concerns non-modifier keys.                                                                                                                                                                                                                                                                                                                                 |
| `bool allows_other_keys_down_while_active`             | Whether to deactivate the override an when another key is pressed down (while the override is active). This only concerns non-modifier keys.                                                                                                                                                                                                                                                                                                       |
| `bool trigger_only_on_trigger_down`                    | Whether to only activate when the trigger key is pressed down while the necessary modifiers are already down. This can be useful for some overrides that would otherwise falsely trigger during quick typing. Setting this to false only works as expected for basic trigger keycodes (keys that appear in the keyboard report), and behaves like setting to true for non-basic trigger keycodes.                                                  |
...
| `bool reregister_trigger_after_completion`             | Whether the trigger key should be registered again after the override is deactivated (only in case it is still physically pressed).                                                                                                                                                                                                                                                                                                                |
