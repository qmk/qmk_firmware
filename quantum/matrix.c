...
static matrix_row_t read_row_state(void);
#else
static void toggle_col(uint8_t col);
static matrix_col_t read_col_state(void);
#endif
...
uint8_t matrix_scan(void) {
    for (int8_t r = MATRIX_ROWS - 1; r >= 0; --r) {
        toggle_row(r);
        matrix_row_t row_state = read_row_state();
        if (matrix_debouncing[r] != row_state) {
            matrix_debouncing[r] = row_state;
...
uint8_t matrix_scan(void) {
    for (int8_t c = MATRIX_COLS - 1; c >= 0; --c) {
        toggle_col(c);
        matrix_col_t col_state = read_col_state();
        if (matrix_debouncing[c] != col_state) {
            matrix_debouncing[c] = col_state;
            debouncing_delay = DEBOUNCING_DELAY;
        }
        toggle_col(c);
    }
