...
static bool PIXEL_FRACTAL(effect_params_t* params) {
    ...
    if (params->init) {
        ...
        for (uint8_t i = led_min; i < led_max; ++i) {
            ...
            // Tag arrays that will be used after scaling by FRACTAL_RATIO
    ...
    if (led_max == DRIVER_LED_TOTAL) {
        for (uint8_t h = 0; h < FRACTAL_HEIGHT; ++h) {
            ...
            led[FRACTAL_SPLIT][h].rgb   = FRACTAL_PIXEL();                  // Randomize inner left column
            led[FRACTAL_SPLIT+1][h].rgb = led[FRACTAL_SPLIT][h].rgb;        // Mirror inner left to right
            led[FRACTAL_SPLIT+1][h].tag = led[FRACTAL_SPLIT][h].tag = true; // Tag both
        }
