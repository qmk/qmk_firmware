...
void process_rgb_matrix_typing_heatmap(keyrecord_t *record) {
    ...
    if (p_row < MATRIX_ROWS) {
      if (m_col < col)
        g_rgb_frame_buffer[p_row][m_col] = qadd8(g_rgb_frame_buffer[p_row][m_col], 13);
      g_rgb_frame_buffer[p_row][col] = qadd8(g_rgb_frame_buffer[p_row][col], 16);
      if (p_col < MATRIX_COLS)
        g_rgb_frame_buffer[p_row][p_col] = qadd8(g_rgb_frame_buffer[p_row][p_col], 13);
    }

    if (m_row < row) {
      if (m_col < col)
        g_rgb_frame_buffer[m_row][m_col] = qadd8(g_rgb_frame_buffer[m_row][m_col], 13);
      g_rgb_frame_buffer[m_row][col] = qadd8(g_rgb_frame_buffer[m_row][col], 16);
      if (p_col < MATRIX_COLS)
        g_rgb_frame_buffer[m_row][p_col] = qadd8(g_rgb_frame_buffer[m_row][p_col], 13);
    }
}
...
bool TYPING_HEATMAP(effect_params_t* params, rgb_config_t* config) {
  ...
  for (int i = led_min; i < led_max; i++) {
    ...
    for (uint8_t j = 0; j < led_count; ++j)
    {
      ...
      HSV hsv = { 170 - qsub8(val, 85), config->sat, scale8((qadd8(170, val) - 170) * 3, config->val) };
