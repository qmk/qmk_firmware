...
enum power_state {
    POWER_STATE_NO_INIT    = 0,  // We're in this state before calling power_init()
    POWER_STATE_INIT       = 1,  // Can consume up to 100mA
    POWER_STATE_CONFIGURED = 2,  // Can consume up to what is specified in configuration descriptor, typically 500mA
    POWER_STATE_SUSPEND    = 3   // Can consume only suspend current
};

extern enum power_state power_state;

void notify_power_state_change_kb(enum power_state power_state);
void notify_power_state_change_user(enum power_state power_state);
