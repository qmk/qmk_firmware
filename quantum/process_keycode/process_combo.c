...
inline bool combo_iter_is_complete(combo_iterator_t *iter) { return (iter->combo_index == COMBO_NULL_INDEX); }
...
void combo_iter_next(combo_iterator_t *iter) {
    ...
    if (iter->combo_index == COMBO_NULL_INDEX) {
        ...
        iter->combo = &key_combos[iter->combo_index];
    }
}
...
inline void combo_iter_remove(combo_iterator_t *iter) {
...
inline void combo_iter_insert(combo_iterator_t *iter, combo_state_t combo_index, combo_t *combo) {
    SET_NEXT_COMBO(combo, iter->combo_index);
    SET_NEXT_COMBO_DIRECT(iter->prev_combo, combo_index);
}

/* Insert combo at head of queue */
inline void combo_queue_insert(combo_state_t *head, combo_state_t combo_index, combo_t *combo) {
