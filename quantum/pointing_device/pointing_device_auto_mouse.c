...
void pointing_device_task_auto_mouse(report_mouse_t mouse_report) {
    ...
    if ((mouse_report.x != 0 || mouse_report.y !=0)) {
        ...
        if(
            !layer_state_is(auto_mouse.config.layer) && 
            timer_elapsed(auto_mouse.timer.delay) > AUTO_MOUSE_DELAY
          ){
            layer_on(auto_mouse.config.layer);
            auto_mouse.timer.delay = 0;
        }
    } else if (
        timer_elapsed(auto_mouse.timer.active) > AUTO_MOUSE_TIME &&
        layer_state_is(auto_mouse.config.layer) &&
        !auto_mouse.status.mouse_key_tracker &&
        !auto_mouse.status.layer_toggled
    ) {
