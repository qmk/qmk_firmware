...
bool webusb_receive_oryx(uint8_t *data, uint8_t length) {
    ...
    switch (command) {
        case ORYX_GET_LAYER: oryx_layer_event(); return true;
        case ORYX_CMD_LIVE_TRAINING:
            {
                uint8_t event[4];
                switch (param[0]) { // 0 for querying, 1 for off, 2 for on
                    case 0: break;
                    case 1: oryx_state_live_training_enabled = false; break;
                    case 2: oryx_state_live_training_enabled = true; break;
                    default: webusb_error(WEBUSB_STATUS_UNKNOWN_COMMAND); return true;
                }
        ...
        case ORYX_CMD_LIVE_UPDATE_GET_MACRO_BUFFER:
            {
                ...
                for (i = 0; i < size; i++) { event[i + 2] = param[i]; }
...
#endif
        default: return webusb_receive_kb(data, length);
    }
}
...
bool process_record_oryx(uint16_t keycode, keyrecord_t *record) {
    ...
    switch (keycode) {
        case MACRO00 ... MACRO15:
            if (record->event.pressed) { dynamic_keymap_macro_send(keycode - MACRO00); }
            return false;
    }
#endif
    return true;
...
void matrix_init_oryx(void) {
    ...
    if (!memcmp(&temp, &firmware, sizeof(uint8_t) * FIRMWARE_VERSION_SIZE)) { eeconfig_init_oryx(); }
#endif
}
